...
const filePath = path.resolve('src/pages/Seller/ProductFormModal.tsx');
...
const stepRegex = /<span className=\{`text-\[10px\] uppercase font-bold tracking-widest rtl:tracking-normal \$\{isActive \? "text-\[#C75C1A\]" : "text-slate-500"\}\`\}>\{t\("Étape"\)\}\{idx \+ 1\}<\/span>\s*<span className=\{`text-xs md:text-sm font-semibold hidden md:block \$\{isActive \? "text-slate-900" : "text-slate-600"\}\`\}>\{step\.title\}<\/span>/g;
...
console.log('Fixes applied to ProductFormModal.tsx via TypeScript script.');
