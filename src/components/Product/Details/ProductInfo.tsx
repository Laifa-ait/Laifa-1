...
import { Star, Info, Store, MapPin, Truck, ShieldCheck, Tag, Sparkles, CalendarDays, Layers, Undo2, Maximize2, FileText, Check, UserPlus, UserCheck, Flame, Clock } from "lucide-react";
...
export const ProductInfo: React.FC<InfoProps> = ({
  product, shop, currentPrice, selectedColor, selectedSize,
  onSelectColor, onSelectSize, isColorOutOfStock, isSizeOutOfStock
}) => {
  ...
  return (
    <div className="space-y-10" dir={isRTL ? "rtl" : "ltr"}>
      <ConfirmModal 
        ...
        message={t("product.details.unfollow_confirm_message") || "Voulez-vous vraiment ne plus suivre cette boutique ?"}
      />
      {/* HEADER SECTION */}
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <span className="px-4 py-1.5 bg-[#121315]/10 text-[#121315] border border-[#121315]/20 rounded-full font-bold text-[10px] rtl:text-[12px] uppercase tracking-widest rtl:tracking-normal">{product.category}</span>
          {product.subcategory && (
            <span className="px-4 py-1.5 bg-[#121315]/5 text-[#121315]/80 border border-[#121315]/10 rounded-full font-bold text-[10px] rtl:text-[12px] uppercase tracking-widest rtl:tracking-normal">{product.subcategory}</span>
          )}
          {product.subSubCategory && (
            <span className="px-4 py-1.5 bg-[#121315]/5 text-[#121315]/80 border border-[#121315]/10 rounded-full font-bold text-[10px] rtl:text-[12px] uppercase tracking-widest rtl:tracking-normal">{product.subSubCategory}</span>
          )}
          {product.gender && product.gender !== 'mixed' && (
            <span className="px-3 py-1.5 bg-stone-100 text-stone-600 rounded-full font-medium text-[10px] rtl:text-[12px] uppercase tracking-wider rtl:tracking-normal">{product.gender}</span>
          )}
          {(product.tags && product.tags.length > 0) && product.tags.map((tag: any, idx: number) => (
             <span key={`tag-${idx}`} className="px-3 py-1.5 bg-stone-100 text-stone-600 rounded-full font-medium text-[10px] rtl:text-[12px] uppercase tracking-wider rtl:tracking-normal">{tag.label || tag}</span>
          ))}
          ...
          {product.brand && (
            <span className="px-3 py-1.5 bg-stone-100 text-stone-600 rounded-full font-medium text-[10px] rtl:text-[12px] uppercase tracking-wider rtl:tracking-normal">{product.brand}</span>
          )}
          {product.condition && (
            <span className="px-3 py-1.5 bg-[#F37021]/10 text-[#F37021] border border-[#F37021]/20 rounded-full font-bold text-[10px] rtl:text-[12px] uppercase tracking-wider rtl:tracking-normal">{product.condition}</span>
          )}
        </div>
        
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#121315] tracking-tight rtl:tracking-normal leading-[1.1] font-display">{productName}</h1>
        
        <div className="flex items-end gap-4 pt-2">
          <div className="text-4xl sm:text-5xl font-black text-[#F37021] tracking-tighter rtl:tracking-normal">{formatPrice(currentPrice)}</div>
          {isProductFlashActive ? (
            <div className="text-xl sm:text-2xl font-semibold text-stone-500/80 line-through mb-1.5">{formatPrice(product.price)}</div>
          ) : product.onSale && (
            <div className="text-xl sm:text-2xl font-semibold text-stone-500/80 line-through mb-1.5">{formatPrice(currentPrice * 1.2)}</div>
          )}
        </div>
      </div>
              ...
              <div className="flex flex-col gap-1.5 border-t sm:border-t-0 sm:border-l border-orange-200/40 pt-3 sm:pt-0 sm:pl-4">
                <div className="flex justify-between text-[11px] font-black text-[#121315]">
                  <span className="text-red-600 animate-pulse flex items-center gap-1">
                    ...
                    {currentLang === 'ar' 
                      ? "بقي منتجات قليلة جداً!" 
                      : "Derniers articles en stock !"}
                  </span>
                  <span className="opacity-80 text-stone-500">92% {currentLang === 'ar' ? "محجوز" : "déjà réservé"}</span>
                </div>
                <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden border border-stone-200/50">
                  <div className="h-full bg-gradient-to-r from-red-600 to-[#F37021] rounded-full" style={{ width: '92%' }} />
                </div>
              </div>
      ...
      {shop && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-white rounded-2xl border border-stone-200 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal mb-0.5">{t("product.details.sold_by") || "Vendu par"}</p>
              <Link to={`/shop/${shop.id}`} className="text-sm font-black text-[#121315] hover:text-[#F37021] transition-colors line-clamp-1">{shop.shopName}</Link>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <Link to={`/shop/${shop.id}`} className="flex-1 sm:flex-none text-center px-4 py-2.5 bg-stone-50 hover:bg-stone-100 text-stone-700 rounded-xl text-xs rtl:text-sm font-bold transition-colors">
      ...
      <div className="bg-white/60 backdrop-blur-md rounded-2xl p-5 border border-stone-200">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#121315]">{t("product.details.seller_word") || "Le mot du créateur"}</h3>
        </div>
        <p className="text-stone-600 leading-relaxed font-medium text-sm sm:text-base whitespace-pre-wrap">{productDescription}</p>
      </div>
      ...
      {(product.colors?.length > 0 || product.sizes?.length > 0) && (
        <div className="grid sm:grid-cols-2 gap-8 p-6 bg-white rounded-3xl border border-stone-200 shadow-sm">
          {product.colors && product.colors.length > 0 && (
              <div className="space-y-4">
                  <h4 className="text-[10px] rtl:text-[12px] font-bold uppercase tracking-wider rtl:tracking-normal text-stone-500 ms-1">{t("product.details.nuances") || "Nuances Disponibles"}</h4>
                  <div className="flex flex-wrap gap-3">
                      {product.colors.map((c: string) => {
                          const matchingColor = PRODUCT_COLORS.find(pc => pc.name.toLowerCase().trim() === c.toLowerCase().trim());
                          ...
                          const isWhiteOrLight = colorHex.toLowerCase() === '#ffffff' || colorHex.toLowerCase() === '#fde68a' || colorHex.toLowerCase() === '#facc15';
                          ...
                          if (hasValidColorValue) {
                            return (
                              ...
                              >
                                ...
                                {isColorOutOfStock(c) && <div className="absolute inset-0 flex items-center justify-center"><div className="w-full h-0.5 bg-red-500 rotate-45"></div></div>}
                              </button>
                            );
                          } else {
                            ...
                            return (
                              ...
                              >
                                {c}
                                {isColorOutOfStock(c) && <div className="absolute inset-0 flex items-center justify-center"><div className="w-full h-[1.5px] bg-red-500 rotate-45"></div></div>}
                              </button>
                            );
                          }
                      })}
           ...
           {product.sizes && product.sizes.length > 0 && (
              <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     <h4 className="text-[10px] rtl:text-[12px] font-bold uppercase tracking-wider rtl:tracking-normal text-stone-500 ms-1">{t("product.details.sizes") || "Tailles / Dimensions"}</h4>
                     <button
                        type="button"
                  ...
                  <div className="flex flex-wrap gap-2">
                      {product.sizes.map((s: string) => (
                          <button key={s} disabled={isSizeOutOfStock(s) && selectedSize !== s} onClick={() => onSelectSize(s)} className={`px-6 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest rtl:tracking-normal transition-all ${selectedSize === s ? "bg-[#121315] text-white shadow-lg shadow-[#121315]/20" : "bg-stone-50 border border-stone-200 text-stone-600 hover:bg-stone-100"} ${isSizeOutOfStock(s) ? "opacity-40 cursor-not-allowed" : ""}`}>{s}</button>
                      ))}
                  </div>
              </div>
          )}
           ...
           <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {detailedAttributes.map((attr, idx) => (
                <div key={idx} className="bg-white p-4 rounded-2xl border border-stone-200">
                  <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal mb-1 line-clamp-1">{attr.label}</p>
                  <p className="text-sm font-black text-[#121315] line-clamp-2">{attr.value} {attr.unit}</p>
                </div>
              ))}
           </div>
        </div>
      ...
      {(product.sku || (product.materials && product.materials.length > 0) || product.season || product.weight || product.dimensions || shop?.avgPreparationTime || shop?.returnPolicy) && (
        <div className="space-y-4 pt-4">
          <h4 className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider rtl:tracking-normal text-stone-500 ms-1">
            <FileText className="w-3.5 h-3.5" /> {t("product.details.fiche_technique") || "Fiche Technique & Garanties OLMART"}
          </h4>
          <div className="bg-white/40 backdrop-blur-md border border-stone-200/60 rounded-3xl p-5 sm:p-6 space-y-5 shadow-sm">
             <div className="grid grid-cols-2 gap-4 text-start">
                {product.sku && (
                   <div className="space-y-1">
                      ...
                      <p className="text-xs rtl:text-sm font-black text-[#121315] font-mono select-all bg-stone-50/80 px-2 py-1 rounded w-max">{product.sku}</p>
                   </div>
                )}
                {product.season && (
                   <div className="space-y-1">
                      <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-widest rtl:tracking-normal flex items-center gap-1">
                         <CalendarDays className="w-3 h-3 text-[#121315]" /> {t("product.details.collection") || "Collection / Saison"}
                      </p>
                ...
                {product.materials && product.materials.length > 0 && (
                   <div className="space-y-1 col-span-2 sm:col-span-1">
                      <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-widest rtl:tracking-normal flex items-center gap-1">
                         <Layers className="w-3 h-3 text-[#121315]" /> {t("product.details.materials") || "Composition & Matière"}
                      </p>
                ...
                {(product.dimensions || product.weight) && (
                   <div className="space-y-1 col-span-2 sm:col-span-1">
                      <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-widest rtl:tracking-normal flex items-center gap-1">
                         <Maximize2 className="w-3 h-3 text-[#121315]" /> {t("product.details.logistics") || "Logistique & Dimensions"}
                      </p>
             ...
             {(product.warranty || shop?.legalStatus || product.preparationTime || shop?.avgPreparationTime || product.returnPolicy || shop?.returnPolicy) && (
                <div className="border-t border-stone-200/50 pt-4 space-y-3 text-start">
                   {product.warranty && (
                      <div className="flex items-start gap-3">
                         ...
                         <div>
                            <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal">{t("Garantie & Support")}</p>
                   ...
                   {shop?.legalStatus && (
                      <div className="flex items-start gap-3">
                         ...
                         <div>
                            <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal">{t("product.details.vendor_status") || "Statut Légal"}</p>
                   ...
                   {(product.preparationTime || shop?.avgPreparationTime) && (
                      <div className="flex items-start gap-3">
                         ...
                         <div>
                            <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal">{t("product.details.prep_time") || "Délai Moyen de Préparation"}</p>
                            <p className="text-xs rtl:text-sm font-extrabold text-[#121315]">{t("product.details.prep_text", { days: product.preparationTime || shop?.avgPreparationTime }) || "Prêt pour expédition"}</p>
                            {product.wilaya && (
                               <p className="text-[10px] rtl:text-[12px] font-medium text-stone-500 mt-0.5">{t("product.details.shipped_from") || "Expédié depuis :"} <span className="font-bold">{product.wilaya}</span></p>
                            )}
                         </div>
                      </div>
                   )}
                   <div className="flex items-start gap-3">
                      ...
                      <div>
                         <p className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal">{t("product.details.return_policy") || "Politique de Retour & Échange"}</p>
                         <p className="text-xs rtl:text-sm font-extrabold text-[#121315] italic">
                           {product.returnPolicy ? (t("product.details.return_14j") || "Retours acceptés sous 14 jours (Politique du produit)") : (shop?.returnPolicy || (t("product.details.return_default") || "Voir les conditions au niveau du vendeur"))}
                         </p>
                      </div>
                   </div>
                </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="flex items-start gap-4 p-5 bg-white border border-stone-200 rounded-2xl">
           ...
           <div>
             <h4 className="text-sm font-bold text-[#121315] mb-1">{t("product.details.national_shipping") || "Livraison Nationale"}</h4>
             <p className="text-xs rtl:text-sm font-medium text-stone-500">{t("product.details.national_shipping_desc") || "Expédition disponible sur les 58 Wilayas d'Algérie."}</p>
           </div>
        </div>
        <div className="flex items-start gap-4 p-5 bg-white border border-stone-200 rounded-2xl">
           ...
           <div>
             <h4 className="text-sm font-bold text-[#121315] mb-1">{t("product.details.secure_purchase") || "Achat Sécurisé"}</h4>
             <p className="text-xs rtl:text-sm font-medium text-stone-500">{t("product.details.secure_purchase_desc") || "Plateforme de confiance. Protection de l'acheteur."}</p>
           </div>
        </div>
      </div>
      ...
      {isSizeGuideOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] max-w-2xl w-full p-8 md:p-10 border border-zinc-100 shadow-2xl relative animate-in zoom-in-95 duration-200">
             <button onClick={() => setIsSizeGuideOpen(false)} className="absolute top-6 right-6 text-stone-400 hover:text-stone-600 font-bold p-2 text-sm cursor-pointer">
                {t("common.close") || "✕ Fermer"}
             </button>
             <h3 className="text-2xl font-black text-[#121315] tracking-tight rtl:tracking-normal mb-4">{t("product.details.size_guide_title") || "📐 Guide des Correspondances de Tailles"}</h3>
             <p className="text-xs rtl:text-sm text-stone-500 mb-6 leading-relaxed">
                {t("product.details.size_guide_desc") || "En Algérie, les articles d'importation (Chine vs. Turquie/EUR) ou de fabrication locale ont des coupes différentes. Référez-vous à ce tableau pour éviter les erreurs de taille :"}
             </p>
             ...
             <div className="bg-amber-50 border border-amber-200/50 p-4 rounded-xl text-[11px] text-[#121315] leading-relaxed font-semibold">
                💡 <strong>{t("product.details.size_guide_tip_title") || "Astuce :"}</strong> {t("product.details.size_guide_tip_content") || "Le standard Turquie correspond parfaitement aux tailles européennes classiques. Pour la Chine, commandez systématiquement une taille au-dessus."}
             </div>
          </div>
        </div>
      )}
