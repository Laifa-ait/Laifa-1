...
export const ProductReviews: React.FC<ReviewsProps> = ({
  comments, userCanReview, submittingReview, newReviewText, 
  setNewReviewText, newReviewStars, setNewReviewStars, onSubmit
}) => {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-8 mt-12 border border-zinc-100 shadow-sm">
      ...
      {comments.length === 0 ? <p className="text-zinc-400">{t("product.reviews_none") || "Aucun avis pour le moment."}</p> : (
        <div className="space-y-6">
          {comments.map((c) => (
            <div key={c.id} className="border-b border-zinc-100 pb-4">
              <p className="font-black text-sm">{c.name}</p>
              <div className="flex text-orange-400">
                {[...Array(c.stars)].map((_, i) => <Star key={i} className="w-3 h-3 fill-current" />)}
              </div>
