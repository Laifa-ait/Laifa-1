...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <div
        className={`bg-[#121315] text-[9px] rtl:text-[11px] sm:text-[10px] rtl:text-[12px] font-medium px-4 sm:px-6 lg:px-12 gap-4 overflow-x-auto whitespace-nowrap scrollbar-hide justify-between items-center text-[#FAF8F5] ${location.pathname === "/" ? "hidden lg:flex" : "hidden"}`}
      >
        <div className="flex items-center mx-auto w-full max-w-[90rem] justify-between">
          <div className="flex items-center gap-6 py-2">
            <span className="flex items-center gap-2 text-[#FAF8F5] uppercase tracking-[0.2em] rtl:tracking-normal">
              <span className="w-1 h-1 bg-[#F37021] shrink-0" />
              {t("trust_delivery")}
            </span>
          ...
          <div className="flex items-center gap-6 py-2">
            <button
              ...
              className="hover:text-[#F37021] transition-colors flex items-center gap-1.5 uppercase tracking-[0.2em] rtl:tracking-normal text-[#FAF8F5] cursor-pointer bg-transparent border-none"
            >
              {t("sell_on_olma")}
            </button>
            <span className="text-white/20">|</span>
      ...
      <nav className="sticky top-0 z-[100] bg-[#FAF8F5]/95 backdrop-blur-md border-b border-[#EBE5DF]/80 shadow-[0_4px_24px_rgba(44,30,22,0.03)] transition-all duration-500 py-3 sm:py-5 relative">
        <div className="absolute top-0 inset-x-0 h-[2px] bg-[#F37021] z-10" />
        <div className="flex items-center px-4 sm:px-6 md:px-8 mx-auto w-full max-w-[90rem] justify-between relative">
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-6 relative lg:w-1/4 shrink-0">
            <button
              ...
              className="flex items-center justify-center text-[#121315] hover:text-[#F37021] transition-all active:scale-95 cursor-pointer relative bg-transparent border-none p-1"
            ...
            <div className="hidden lg:block relative">
              <button
                ...
                className="flex items-center gap-1.5 text-[11px] font-bold text-[#121315] hover:text-[#F37021] transition-all duration-300 py-1.5 px-3.5 uppercase tracking-widest rtl:tracking-normal bg-white border border-[#EBE5DF]/80 rounded-full hover:shadow-[0_4px_12px_rgba(44,30,22,0.06)] cursor-pointer"
                title={t("choose_language") || "Choisir la langue / اختر اللغة"}
                id="language-select-desktop-btn"
              >
                <Globe className="w-3.5 h-3.5 text-[#121315]/80" />
                ...
                <>
                  ...
                  <div className="absolute top-full right-0 mt-3 bg-[#FAF8F5] border border-[#EBE5DF]/80 shadow-[0_20px_40px_rgba(44,30,22,0.12)] z-[60] py-1.5 rounded-2xl min-w-[130px] overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200">
                    {[
                      ...
                      >
                        <span>{l.name}</span>
                        {lang === l.code && <span className="w-1.5 h-1.5 rounded-full bg-[#F37021]" />}
                      </button>
                    ))}
                  </div>
                </>
            ...
            <button
              ...
              className="hidden lg:flex items-center justify-center text-[#121315] hover:text-[#F37021] transition-all active:scale-95 cursor-pointer relative bg-transparent border-none p-1"
            >
              <Heart className="w-5 h-5 sm:w-6 sm:h-6 stroke-[1.5]" />
              {wishlist.length > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#F37021] text-white flex items-center justify-center text-[9px] rtl:text-[11px] font-bold border-2 border-[#FAF8F5]">
                  {wishlist.length}
                </span>
              )}
            </button>
            ...
            >
              ...
              {cart.reduce((acc, i) => acc + i.quantity, 0) > 0 && (
                <span className="absolute -top-1 -right-2 w-5 h-5 rounded-full bg-[#F37021] text-white flex items-center justify-center text-[10px] rtl:text-[12px] font-bold border-2 border-[#FAF8F5]">
                  {cart.reduce((acc, i) => acc + i.quantity, 0)}
                </span>
              )}
            </button>

            {/* Hamburger on Right - Replaces Profile Icon */}
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="flex items-center justify-center transition-colors text-[#121315] hover:text-[#F37021] active:scale-95 cursor-pointer bg-transparent border-none p-1"
            >
                    ...
                    <>
                      ...
                      <div className="py-2">
                        <button
                          ...
                          className="w-full h-11 flex items-center px-5 text-[11px] uppercase tracking-widest rtl:tracking-normal text-[#121315] hover:bg-[#EBE5DF]/50 transition-colors gap-3 bg-transparent border-none cursor-pointer"
                        >
                        ...
                        {userProfile?.role === "seller" && (
                          <button
                            ...
                            className="w-full h-11 flex items-center px-5 text-[11px] uppercase tracking-widest rtl:tracking-normal text-[#F37021] hover:bg-[#F37021]/5 transition-colors gap-3 bg-transparent border-none cursor-pointer"
                          >
                      ...
                      <div className="pt-2 border-t border-[#EBE5DF] px-2 mb-2">
                        <button
                          onClick={() => {
                            logout();
                            setIsUserDropdownOpen(false);
                          }}
                          className="w-full h-10 flex items-center justify-center text-[10px] rtl:text-[12px] font-normal uppercase tracking-[0.2em] text-[#121315]/60 hover:text-[#F37021] transition-colors bg-transparent border-none cursor-pointer"
                        >
