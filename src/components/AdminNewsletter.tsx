...
export default function AdminNewsletter({ auth }: any) {
    ...
    const newBlock: ContentBlock = {
      ...
      data:
        type === "text"
          ? { text: "" }
          : type === "image"
            ? { url: "", align: "center", width: 100 }
            : type === "button"
              ? { text: "Cliquez ici", link: "#", align: "center" }
              : { height: 20 },
    };
  ...
  const RenderPreview = () => (
    <div
      className={`mx-auto bg-white shadow-2xl overflow-hidden transition-all duration-300 ${previewDevice === "mobile" ? "max-w-[375px] rounded-[3rem] border-[8px] border-zinc-900" : "max-w-4xl"}`}
    >
      <div className="bg-zinc-50 p-4 border-b border-zinc-100 mb-6">
        <p className="text-[10px] rtl:text-[12px] text-zinc-400 uppercase font-bold tracking-widest rtl:tracking-normal mb-1">
          {t("De:")}
          {settings.senderName} {t("&lt;")}
          {settings.senderEmail}
          {t("&gt;")}
        </p>
        <p className="text-xs rtl:text-sm font-bold text-zinc-800">
          {t("Objet:")}
          {campData.subject || "(Pas d'objet)"}
        </p>
      </div>
        ...
        {campData.blocks.map((block) => (
          <div key={block.id} className="w-full">
            ...
            {block.type === "image" && block.data.url && (
              <div
                className={`flex ${block.data.align === "left" ? "justify-start" : block.data.align === "right" ? "justify-end" : "justify-center"}`}
              >
                <img
                  loading="lazy"
                  src={block.data.url}
            ...
            {block.type === "button" && (
              ...
              >
                <a
                  ...
                  className="bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold hover:brightness-110 transition-all text-sm inline-block"
                >
                  {block.data.text}
                </a>
              </div>
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {alert && (
        <div
          className={`fixed bottom-6 right-6 z-50 p-4 rounded-xl shadow-lg border flex items-center gap-3 ${alert.type === "success" ? "bg-emerald-50 text-emerald-800 border-emerald-200" : "bg-red-50 text-red-800 border-red-200"}`}
        >
      ...
      <div className="bg-white rounded-3xl p-4 lg:p-6 border border-zinc-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {activeTab === "campaigns" && currentCampaignId && (
          <button
            onClick={resetForm}
            className="text-xs rtl:text-sm font-bold text-zinc-400 hover:text-emerald-600 flex items-center gap-2 px-4 py-2 bg-zinc-50 rounded-xl transition-colors"
          >
            <Plus className="w-4 h-4" /> {t("Nouvelle Campagne")}
          </button>
        )}
      </div>
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[2.5rem] border border-zinc-100">
          ...
          <p className="text-[10px] rtl:text-[12px] text-zinc-400 font-bold uppercase tracking-widest rtl:tracking-normal">
            {t("Initialisation du studio...")}
          </p>
        </div>
      )}
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <div
                  key={i}
                  className="bg-white rounded-[2rem] p-6 border border-zinc-100 flex flex-col items-center text-center"
                >
        ...
        <div className="space-y-6">
          <div className="flex bg-white rounded-[2rem] p-2 border border-zinc-100 w-fit">
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-[10px] rtl:text-[12px] font-bold uppercase tracking-widest rtl:tracking-normal flex items-center gap-2 transition-all ${viewMode === "edit" ? "bg-zinc-900 text-white shadow-xl" : "text-zinc-400"}`}
            >
              <Type className="w-3.5 h-3.5" /> {t("Éditeur Studio")}
            </button>
            <button
              onClick={() => setViewMode("preview")}
              className={`px-6 py-2.5 rounded-xl text-[10px] rtl:text-[12px] font-bold uppercase tracking-widest rtl:tracking-normal flex items-center gap-2 transition-all ${viewMode === "preview" ? "bg-zinc-900 text-white shadow-xl" : "text-zinc-400"}`}
            >
              <Eye className="w-3.5 h-3.5" /> {t("Aperçu Réel")}
            </button>
          </div>
                ...
                <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 border border-zinc-100 space-y-8">
                  <div>
                    ...
                    <div className="space-y-4">
                      <input
                        type="text"
                        placeholder={t("Nom interne (ex: Promo été 2024)") || "Nom interne (ex: Promo été 2024)"}
                        value={campData.title || ""}
                        onChange={(e) => setCampData({ ...campData, title: e.target.value })}
                        className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-sm focus:bg-white focus:ring-4 focus:ring-emerald-50 transition-all outline-none"
                      />
                      <input
                        type="text"
                        placeholder={t("Sujet de l'email") || "Sujet de l'email"}
                        value={campData.subject || ""}
                        onChange={(e) => setCampData({ ...campData, subject: e.target.value })}
                        className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-sm font-bold focus:bg-white focus:ring-4 focus:ring-emerald-50 transition-all outline-none"
                      />
                      <div className="flex gap-4">
                        <select
                          value={campData.targeting || ""}
                          onChange={(e) => setCampData({ ...campData, targeting: e.target.value })}
                          className="flex-1 bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-xs rtl:text-sm font-bold uppercase tracking-widest rtl:tracking-normal outline-none appearance-none cursor-pointer"
                        >
                  ...
                  <div>
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] rtl:tracking-normal text-zinc-400 mb-6">
                      {t("Studio de Composition")}
                    </h3>
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                disabled={index === 0}
                                className="p-2 hover:bg-white rounded-xl disabled:opacity-20"
                              >
                                <MoveUp className="w-3.5 h-3.5" />
                              </button>
                              <button
                                onClick={() => moveBlock(index, "down")}
                                disabled={index === campData.blocks.length - 1}
                                className="p-2 hover:bg-white rounded-xl disabled:opacity-20"
                              >
                          ...
                          {block.type === "text" && (
                            <textarea
                              ...
                              placeholder={t("Dites quelque chose de génial...") || "Dites quelque chose de génial..."}
                              className="w-full bg-white border border-zinc-100 rounded-xl p-4 text-xs rtl:text-sm font-medium focus:ring-4 focus:ring-emerald-50 outline-none min-h-[100px] resize-none"
                            />
                          )}
                          ...
                          {block.type === "image" && (
                            <div className="grid grid-cols-2 gap-3">
                              <div className="col-span-2 flex gap-2">
                                <input
                                  placeholder={t("URL Image") || "URL Image"}
                                  value={block.data.url || ""}
                                  onChange={(e) => updateBlock(block.id, { ...block.data, url: e.target.value })}
                                  className="flex-1 bg-white border border-zinc-100 rounded-xl px-4 py-3 text-[11px] font-bold outline-none"
                                />
                                <button
                              ...
                              <select
                                ...
                                onChange={(e) => updateBlock(block.id, { ...block.data, align: e.target.value })}
                                className="bg-white border border-zinc-100 rounded-xl px-4 py-2 text-[10px] rtl:text-[12px] font-bold outline-none"
                              >
                              ...
                              <input
                                type="number"
                                placeholder={t("Largeur %") || "Largeur %"}
                                value={block.data.width || ""}
                                onChange={(e) =>
                                  updateBlock(block.id, { ...block.data, width: parseInt(e.target.value) })
                                }
                                className="bg-white border border-zinc-100 rounded-xl px-4 py-2 text-[10px] rtl:text-[12px] font-bold outline-none"
                              />
                            </div>
                          ...
                          {block.type === "button" && (
                            <div className="grid grid-cols-2 gap-3">
                              <input
                                placeholder={t("Texte") || "Texte"}
                                value={block.data.text || ""}
                                onChange={(e) => updateBlock(block.id, { ...block.data, text: e.target.value })}
                                className="w-full bg-white border border-zinc-100 rounded-xl px-4 py-2 text-[10px] rtl:text-[12px] font-bold outline-none"
                              />
                              <input
                                placeholder={t("Lien URL") || "Lien URL"}
                                value={block.data.link || ""}
                                onChange={(e) => updateBlock(block.id, { ...block.data, link: e.target.value })}
                                className="w-full bg-white border border-zinc-100 rounded-xl px-4 py-2 text-[10px] rtl:text-[12px] font-bold outline-none"
                              />
                            </div>
                          )}
                      ...
                      {campaigns.map((c) => (
                        <div key={c.id} className="p-8 hover:bg-zinc-50/50 transition-colors group">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest rtl:tracking-normal ${c.status === "sent" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                            >
                              {c.status}
                            </span>
                          </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                <div className="flex justify-center gap-4">
                  <button
                    ...
                    className={`p-4 rounded-2xl border transition-all ${previewDevice === "desktop" ? "bg-zinc-900 text-white border-zinc-900 shadow-xl" : "bg-white text-zinc-400 border-zinc-100"}`}
                  >
                    <Monitor className="w-5 h-5" />
                  </button>
                  <button
                    onClick={() => setPreviewDevice("mobile")}
                    className={`p-4 rounded-2xl border transition-all ${previewDevice === "mobile" ? "bg-zinc-900 text-white border-zinc-900 shadow-xl" : "bg-white text-zinc-400 border-zinc-100"}`}
                  >
              ...
              <tbody className="divide-y divide-zinc-50">
                {subscribers.map((s) => (
                  <tr key={s.id} className="hover:bg-zinc-50/50">
                    ...
                    <td className="px-8 py-5">
                      <span className="text-[10px] rtl:text-[12px] font-bold uppercase tracking-widest rtl:tracking-normal text-zinc-500 py-1 px-3 bg-zinc-100 rounded-full">
                        {s.group}
                      </span>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${s.status === "subscribed" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-rose-500"}`}
                        />
                        <span
                          className={`text-[10px] rtl:text-[12px] font-black uppercase tracking-widest rtl:tracking-normal ${s.status === "subscribed" ? "text-emerald-700" : "text-rose-700"}`}
                        >
                          {s.status}
                        </span>
                      </div>
                    ...
                    <td className="px-8 py-5 text-right">
                      <button className="p-2.5 text-zinc-300 hover:text-rose-500 transition-colors">
