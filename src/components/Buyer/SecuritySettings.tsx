...
export const SecuritySettings: React.FC<SecuritySettingsProps> = ({ currentUser }) => {
  ...
  return (
    <div className="space-y-8" id="security-settings-module">
      <div>
        <h3 className="font-black text-xl text-zinc-900 tracking-tight rtl:tracking-normal">{t("Configuration de Sécurité")}</h3>
        <p className="text-zinc-500 text-xs rtl:text-sm font-medium">{t("Gérez vos accès et sécurisez votre connexion d'e-commerce.")}</p>
      </div>

      <div className="grid md:grid-cols-2 gap-8">
        {/* Email Form */}
        <form onSubmit={triggerEmailUpdate} className="bg-white border border-zinc-100 p-8 rounded-3xl space-y-6 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600">
              <Mail className="w-4 h-4" />
            </div>
            <h4 className="font-black text-xs rtl:text-sm uppercase tracking-wider rtl:tracking-normal text-zinc-900">{t("Adresse de Connexion")}</h4>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-400 uppercase tracking-wider rtl:tracking-normal">{t("Adresse E-mail actuelle et valide")}</label>
            <input
              type="email"
              required
              value={email}
              ...
              placeholder={t("Ex: laifa.ait@gmail.com") || "Ex: laifa.ait@gmail.com"}
            />
          </div>
          ...
          <div className="pt-2">
            ...
            >
              {t("Mettre à jour l'e-mail")}</button>
          </div>
        </form>

        {/* Password Form */}
        <form onSubmit={triggerPasswordUpdate} className="bg-white border border-zinc-100 p-8 rounded-3xl space-y-6 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-xl bg-orange-50 flex items-center justify-center text-orange-600">
              <Key className="w-4 h-4" />
            </div>
            <h4 className="font-black text-xs rtl:text-sm uppercase tracking-wider rtl:tracking-normal text-zinc-900">{t("Changer de Mot de passe")}</h4>
          </div>

          <div className="space-y-4">
            <div className="space-y-2 relative">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-400 uppercase tracking-wider rtl:tracking-normal">{t("Nouveau Mot de passe")}</label>
              <input
            ...
            <div className="space-y-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-400 uppercase tracking-wider rtl:tracking-normal font-sans">{t("Confirmer le Mot de passe")}</label>
              <input
                type={showPassToggle ? "text" : "password"}
                required
          ...
          <div className="pt-2">
            ...
            >
              {t("Changer mon mot de passe")}</button>
          </div>
        </form>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-zinc-950/60 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-[2rem] border border-zinc-150 p-8 max-w-md w-full relative shadow-2xl space-y-6">
            ...
            <div className="text-center space-y-3">
              ...
              <h3 className="font-extrabold text-lg text-zinc-900 tracking-tight rtl:tracking-normal">{t("Vérification de Sécurité")}</h3>
              <p className="text-zinc-500 text-xs rtl:text-sm">
                {t("Par mesure de sécurité hautement requise par Olma, veuillez confirmer votre mot de passe de connexion actuel avant de modifier vos informations d'identité sensibles.")}</p>
            </div>

            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <label className="text-[10px] rtl:text-[12px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal block leading-none">{t("Votre mot de passe actuel")}</label>
                <input
                  type="password"
                  required
