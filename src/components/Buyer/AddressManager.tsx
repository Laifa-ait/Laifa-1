...
export const AddressManager: React.FC<AddressManagerProps> = ({ currentUser, userProfile }) => {
  ...
  const handleAddAddress = async (e: React.FormEvent) => {
    ...
    if (!formData.isShipping && !formData.isBilling) {
      return toast.error("La destination de l'adresse doit être au moins configurée pour la livraison ou la facturation.");
    }
  ...
  return (
    <div className="space-y-8" id="address-manager-module">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-black text-xl text-zinc-900 tracking-tight rtl:tracking-normal text-start">{t("Mon Carnet d'Adresses")}</h3>
          <p className="text-zinc-500 text-xs rtl:text-sm text-start">{t("Configurez vos adresses de livraison principales et de facturation légale.")}</p>
        </div>
        {!showAddForm && (
          <button
            onClick={() => {
          ...
          >
            <Plus className="w-4 h-4" /> {t("Nouvelle adresse")}</button>
        )}
      </div>

      {showAddForm && (
        <form onSubmit={handleAddAddress} className="bg-zinc-50 border border-zinc-150 rounded-3xl p-8 space-y-6 animate-pulse-once text-start">
          <h4 className="font-extrabold text-sm text-zinc-900 uppercase tracking-widest rtl:tracking-normal flex items-center gap-2">
            <Compass className="w-5 h-5 text-zinc-500 animate-spin-slow" /> {t("Ajouter une adresse structurée")}</h4>
          
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Nom du destinataire / Raison sociale")}</label>
              <input
                type="text"
                required
                value={formData.name}
            ...
            <div className="space-y-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Téléphone de livraison")}</label>
              <input
                type="tel"
                required
            ...
            <div className="space-y-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Wilaya")}</label>
              <select
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Daïra / Arrondissement")}</label>
                <button
                  type="button"
                  onClick={() => {
              ...
              {manualDaira ? (
                ...
                >
                  {currentDairas.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Commune / Baladia")}</label>
                <button
                  type="button"
                  onClick={() => {
            ...
            <div className="space-y-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Code Postal (Algérien - 5 Chiffres)")}</label>
              <input
                type="text"
                required
                maxLength={5}
            ...
            <div className="space-y-2 sm:col-span-2">
              <label className="text-[10px] rtl:text-[12px] font-bold text-zinc-650 uppercase tracking-wider rtl:tracking-normal block">{t("Rue / Bâtiment / N° de porte (Détails de livraison)")}</label>
              <textarea
                rows={2}
                required
                value={formData.rue}
                ...
                placeholder={t("Ex: Cité 500 Logements, Bâtiment C, Appartement 12") || "Ex: Cité 500 Logements, Bâtiment C, Appartement 12"}
              />
            </div>

            {/* Address Roles Logic (Gifting and B2B enabled) */}
            <div className="sm:col-span-2 space-y-3 pt-3">
              <label className="text-[10px] rtl:text-[12px] font-bold text-stone-500 uppercase tracking-wider rtl:tracking-normal block leading-none text-start">{t("Usage de l'adresse")}</label>
              <div className="flex gap-6">
                <label className="flex items-center gap-2.5 cursor-pointer">
                  ...
                  />
                  <span className="text-xs rtl:text-sm font-bold text-zinc-700 select-none">{t("Adresse de Livraison (Shipping)")}</span>
                </label>
                ...
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 rounded text-zinc-900 border-zinc-300 focus:ring-zinc-500 accent-zinc-900 cursor-pointer"
                  />
                  <span className="text-xs rtl:text-sm font-bold text-zinc-700 select-none">{t("Adresse de Facturation (Billing)")}</span>
                </label>
              </div>
            </div>
          </div>
      ...
      {addresses.length === 0 ? (
        <div className="bg-zinc-50 rounded-3xl p-12 text-center border border-zinc-100 animate-pulse-once">
          ...
          <p className="text-zinc-400 text-xs rtl:text-sm mt-1">{t("Ajoutez une adresse pour débloquer le service de livraison structuré d'Olma.")}</p>
        </div>
      ) : (
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
                            ...
                            {!addr.isDefault && (
                              ...
                              >
                                {t("Définir par défaut")}</button>
                            )}
                          </div>

                          <div className="space-y-1.5 text-zinc-700">
                            <p className="font-black text-sm text-zinc-900 tracking-tight rtl:tracking-normal">{addr.name || userProfile?.displayName}</p>
                            <p className="font-extrabold text-[11px] text-zinc-800 uppercase tracking-widest rtl:tracking-normal">
                              {addr.wilaya} • {addr.daira ? `${addr.daira} • ` : ""}{addr.commune}
                            </p>
                            <p className="text-xs rtl:text-sm font-semibold text-zinc-500">{t("CP:")}{addr.codePostal || '16000'}</p>
