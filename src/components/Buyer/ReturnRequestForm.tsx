...
export const ReturnRequestForm: React.FC<ReturnRequestProps> = ({ orderId, onClose, onSubmit }) => {
   ...
   return (
      ...
      >
         <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] p-6 w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button type="button" onClick={onClose} className="absolute top-4 end-4 p-2 text-zinc-400 hover:text-zinc-600"><X className="w-5 h-5"/></button>
            ...
            <div className="space-y-4">
               <div>
                  ...
                  <select value={reason || ''} onChange={e => setReason(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl font-medium text-zinc-800">
               ...
               <div>
                  ...
                  <textarea value={details || ''} onChange={e => setDetails(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl h-24 font-medium text-zinc-800 resize-none" placeholder={t("Expliquez en détail le problème...") || "Expliquez en détail le problème..."} />
               </div>

               <div>
                  <label className="block text-xs rtl:text-sm font-bold text-zinc-500 uppercase mb-2">{t("Preuves (Photos)")}</label>
                  <input 
                     type="file" 
                     ref={fileInputRef} 
                  ...
                  {photos.length < 5 && (
                     ...
                     >
                        <Camera className="w-5 h-5" /> {t("Ajouter des photos (Max 5)")}</button>
                  )}
               </div>
            </div>

            <button disabled={loading} className="w-full mt-6 py-4 flex items-center justify-center gap-2 bg-[#F37021] text-white rounded-2xl font-bold hover:bg-[#A94320] transition-colors disabled:opacity-50">
