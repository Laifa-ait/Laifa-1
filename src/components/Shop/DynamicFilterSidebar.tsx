...
export const DynamicFilterSidebar: React.FC<DynamicFilterSidebarProps> = ({ 
  ...
  const handleCheckboxChange = (filterId: string, option: string, isChecked: boolean) => {
    ...
    if (isChecked) {
      ...
      handleFilterChange(filterId, currentList.filter(v => v !== option));
    }
  };
      ...
      case 'radio':
        return (
          <div className="space-y-2 mt-2">
            ...
            >
              <option value="">{t('all_the', 'Tous les')} {t(filter.label, filter.label).toLowerCase()}</option>
              {filter.options?.map((opt: any) => {
                const label = typeof opt === 'string' ? opt : opt.label;
                const value = typeof opt === 'string' ? opt : opt.value;
                return (
                  <option key={value} value={value}>{t(label, label) as string}</option>
                );
              })}
            </select>
          </div>
            ...
            {filter.options?.map((opt: any) => {
              ...
              return (
                <label key={value} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <div className={`w-4 h-4 rounded flex items-center justify-center border transition-colors ${isChecked ? 'bg-blue-900 border-blue-900' : 'bg-white border-slate-300 group-hover:border-blue-900'}`}>
                    {isChecked && <Check className="w-3 h-3 text-white" />}
                  </div>
                  <span className={`text-[13px] font-medium transition-colors ${isChecked ? 'text-blue-900 font-bold' : 'text-slate-600 group-hover:text-slate-900'}`}>
    ...
    <div className="flex flex-col h-full bg-white lg:bg-transparent">
      ...
      <div className="lg:hidden flex items-center justify-between p-4 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-slate-900">{t('filter_filters', 'Filtres')} {categoryDef.name}</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-0 bg-white lg:bg-transparent">
        <div className="lg:border lg:border-slate-200 lg:bg-white lg:rounded-2xl overflow-hidden p-4">
          <div className="hidden lg:flex items-center gap-2 mb-6 pb-4 border-b border-slate-100">
            ...
            <span className="font-black text-sm uppercase tracking-wider rtl:tracking-normal text-slate-900">{t('dynamic_filters', 'Filtres Dynamiques')}</span>
          </div>
