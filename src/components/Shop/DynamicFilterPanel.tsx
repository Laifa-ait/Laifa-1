...
export const DynamicFilterPanel: React.FC<DynamicFilterPanelProps> = ({
  ...
  const activeFacetsConfig = useMemo(() => {
    ...
    const showColor = activeCategory === "Mode" || activeCategory === "Maison & Déco" || activeCategory === "Supermarché" || activeCategory === "Tous";
    const showMaterial = activeCategory === "Supermarché" || activeCategory === "Maison & Déco" || activeCategory === "Beauté & Santé" || activeCategory === "Tous";
    const showBrand = activeCategory === "Électronique" || activeCategory === "Électroménager" || activeCategory === "Beauté & Santé" || activeCategory === "Scolaire & Bureau" || activeCategory === "Tous";
  ...
  const handlePriceChange = (index: 0 | 1, val: string) => {
    ...
    const newRange: [number, number] = index === 0 
      ? [num, currentPriceRange[1]] 
      : [currentPriceRange[0], num];
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-[#EBE5DF] flex items-center justify-between bg-[#FAF8F5]/60 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-black text-sm tracking-tight rtl:tracking-normal text-[#121315] uppercase">{t('facet_filtering', 'Filtrage par Facettes')}</h3>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 custom-scrollbar">
              ...
              <div className="p-4 bg-orange-50/50 rounded-2xl border border-orange-100/70 text-[11px] font-bold text-stone-700 space-y-1">
                ...
                <p>{t('secure_delivery_desc', 'Aucun paiement en ligne requis. Commandez simplement avec vos spécifications et réglez l\'artisan à la livraison en main propre !')}</p>
              </div>

              {/* SECTION: Price Budget limits */}
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em] ms-0.5">{t('desired_budget_dzd', 'Budget souhaité (DZD)')}</h4>
                  <button 
                    onClick={() => setPriceRange([availableFacets.minPrice, Math.min(availableFacets.maxPrice, Math.max(availableFacets.minPrice + 1000, 45000))])}
                ...
                <div className="space-y-4 bg-white p-5 rounded-3xl border border-[#EBE5DF]/60">
                  <div className="relative pt-2">
                    ...
                    <div className="flex justify-between text-[10px] rtl:text-[12px] text-stone-600 font-bold mt-2">
                      ...
                      <span>{t('max_orderable', 'Max commandable')}: {new Intl.NumberFormat('en-US').format(selectedFacets.priceRange[1])}</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[8px] font-black text-stone-500 uppercase tracking-widest rtl:tracking-normal mb-1">{t('medium_min', 'MÉDIUM MIN')}</label>
                      <div className="relative">
                        ...
                        />
                        <span className="absolute end-3.5 top-1/2 -translate-y-1/2 text-[9px] rtl:text-[11px] font-black text-stone-500 bg-stone-100 border border-[#EBE5DF]/80 px-2 py-1 rounded-md">{t('da', 'DA')}</span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-[8px] font-black text-stone-500 uppercase tracking-widest rtl:tracking-normal mb-1">{t('plafond_max', 'PLAFOND MAX')}</label>
                      <div className="relative">
                        <input
                          type="text"
              ...
              {availableFacets.wilayas.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em]">{t('region_wilaya', 'Région / Wilaya')}</h4>
                  </div>
                  <div className="flex flex-wrap gap-2.5">
                    {availableFacets.wilayas.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          {t(`wilaya_${opt.value}`, opt.value)} <span className={`ms-1 text-[8px] ${isSelected ? 'text-white/80' : 'text-stone-500 font-bold'}`}>({opt.count})</span>
                        </button>
                      );
                    })}
                  </div>
              ...
              {activeFacetsConfig.showSize && availableFacets.sizes.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em]">{t('available_sizes', 'Tailles disponibles (Variantes)')}</h4>
                  </div>
              ...
              {activeFacetsConfig.showMaterial && availableFacets.materials.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em]">{t('artisanal_materials', 'Matières Artisanales')}</h4>
                  </div>
                  <div className="flex flex-wrap gap-2.5">
              ...
              {activeFacetsConfig.showColor && availableFacets.colors.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em]">{t('shades_colors', 'Teintes & Couleurs')}</h4>
                  </div>
                  <div className="flex flex-wrap gap-2.5">
              ...
              {activeFacetsConfig.showBrand && availableFacets.brands.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-[10px] rtl:text-[12px] font-black uppercase text-stone-600 tracking-[0.14em] ms-0.5">{t('brand_manufacturer', 'Marque / Fabricant')}</h4>
                  <div className="flex flex-wrap gap-2.5">
              ...
              {totalCount === 0 && (
                <div className="py-6 px-4 bg-red-400/5 text-center rounded-2xl border border-red-200/50 space-y-1">
                  <p className="text-xs rtl:text-sm font-black text-rose-700 uppercase tracking-widest rtl:tracking-normal">{t('no_match', 'Aucune correspondance')}</p>
                  <p className="text-[10px] rtl:text-[12px] text-stone-500 font-bold">{t('expand_options_desc', "Essayez d'élargir vos options pour débloquer de magnifiques résultats.")}</p>
                </div>
              )}
            </div>
            ...
            <div className="p-6 bg-white border-t border-[#EBE5DF] shrink-0">
              ...
              >
                <span>
                  {t('apply_filter', 'Appliquer le filtre')} ({totalCount} {totalCount > 1 ? t('articles', 'articles') : t('article', 'article')})
                </span>
              </button>
            </div>
          </motion.div>
