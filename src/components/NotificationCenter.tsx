...
interface NotificationItem {
  ...
  description: string;
  time: string;
  type: "message" | "status_shipped" | "status_delivered" | "support" | "coupon" | "system" | "new_order" | "dispute" | "withdrawal";
  link: string;
  read: boolean;
}
...
export const NotificationCenter: React.FC = () => {
  ...
  useEffect(() => {
      ...
      setOrderNotifications([
        {
          ...
          description: t("notif.welcome_desc") || "La marketplace des créations de nos 58 Wilayas. Vos achats sont protégés.",
          time: t("A l'instant"),
          type: "system",
          link: "/shop",
          read: false,
    ...
    const unsubscribeDirect = onSnapshot(directNotifsQuery, (snapshot) => {
      ...
      const docs = snapshot.docs.map(doc => {
        ...
        const titleText = data.title 
          ? (data.title[lang] || data.title.fr || data.title) 
          : "Notification";
        const descText = data.message 
          ? (data.message[lang] || data.message.fr || data.message) 
          : "";
    ...
    const unsubscribeCoupons = onSnapshot(couponsQuery, (snapshot) => {
      const docs = snapshot.docs.map(doc => {
        ...
        return {
          ...
          description: t("notification.new_coupon_desc", "Profitez de {{discount}} de réduction ! (Min. {{min}} DA)", { discount: discountDisplay, min: data.minOrderValue || 0 }),
          time: timeStr,
          type: "coupon",
          link: "/shop",
          ...
          >
            <div className="px-6 py-4 border-b border-[#EBE5DF]/50 flex items-center justify-between bg-[#FAF8F5]/60 shrink-0">
              <div>
                ...
                <p className="text-[9px] rtl:text-[11px] font-black uppercase text-[#F37021] tracking-widest rtl:tracking-normal mt-0.5">{t("Mélodieux & Direct")}</p>
              </div>
              {unreadCount > 0 && (
                <button
            ...
            <div className="max-h-[360px] overflow-y-auto divide-y divide-[#EBE5DF]/30">
              {notifications.length === 0 ? (
                <div className="p-8 text-center space-y-4">
                  ...
                  <p className="text-[10px] rtl:text-[12px] font-black uppercase tracking-wider rtl:tracking-normal text-[#121315]/60">{t("Aucune alerte")}</p>
                </div>
              ) : (
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0 space-y-1">
                                      <div className="flex items-start justify-between gap-2">
                                        <p className={`text-xs rtl:text-sm tracking-tight rtl:tracking-normal truncate leading-tight ${item.read ? "text-[#121315] font-bold" : "text-[#121315] font-extrabold"}`}>
                                          {item.title}
                                        </p>
            ...
            <div className="p-3.5 border-t border-[#EBE5DF]/50 bg-[#FAF8F5]/60 text-center shrink-0">
              <button
                onClick={() => {
                  setIsOpen(false);
                  navigate(currentUser ? (userProfile?.role === "seller" ? "/dashboard/seller" : "/dashboard/buyer") : "/auth");
                }}
