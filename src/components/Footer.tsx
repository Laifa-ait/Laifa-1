...
export const Footer: React.FC<{ isHomepage: boolean }> = ({ isHomepage }) => {
  ...
  return (
    <footer
      className={`custom-dark-footer bg-[#121315] text-white pt-16 sm:pt-20 sm:pb-8 border-t border-[#121315]/20 ${isHomepage ? "pb-24" : "pb-8"}`}
    >
      <div className="w-full max-w-[90rem] mx-auto px-4 sm:px-6 md:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 pb-4">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-2xl font-black uppercase tracking-tighter rtl:tracking-normal">
              {t("OLMA")}
              <span className="text-orange-500">{t("RT")}</span>
            </span>
          </div>
        ...
        {/* Column 2: Newsletter */}
        <div className="space-y-6">
          <h5 className="text-[10px] rtl:text-[12px] font-black uppercase tracking-[0.2em] text-orange-500">
            {t("newsletter") || "Newsletter"}
          </h5>
          <div className="space-y-4">
            <p className="text-[10px] rtl:text-[12px] text-white/40 font-black uppercase tracking-widest rtl:tracking-normal">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex flex-col gap-2">
              <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] rtl:text-[11px] font-black uppercase tracking-widest rtl:tracking-normal hover:bg-white/10 transition-all text-start">
                {t("App Store")}
              </button>
              <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] rtl:text-[11px] font-black uppercase tracking-widest rtl:tracking-normal hover:bg-white/10 transition-all text-start">
                {t("Google Play")}
      ...
      <div className="w-full max-w-[90rem] mx-auto px-4 sm:px-6 md:px-8 mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6 text-xs rtl:text-sm text-white/60 font-medium">
        ...
        <div className="flex items-center gap-4">
          <span className="text-white/40 uppercase tracking-widest rtl:tracking-normal text-[9px] rtl:text-[11px] font-black">
