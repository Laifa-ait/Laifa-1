...
export const MonthlyUpdateBanner: React.FC = () => {
    ...
    return (
        <>
                            ...
                            <div className="flex items-start sm:items-center gap-3 flex-1">
                                ...
                                <div>
                                    <div className="text-[10px] rtl:text-[12px] font-black text-amber-300 uppercase tracking-widest rtl:tracking-normal mb-0.5">
                                        {currentLang === 'ar' ? 'تحديثات الشهر' : currentLang === 'en' ? 'Monthly Update' : 'Mise à jour'} • {latestUpdate.month || latestUpdate.id}
                                    </div>
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm" dir={currentLang === 'ar' ? 'rtl' : 'ltr'}>
                        <motion.div
                        ...
                        >
                            <div className="flex items-center justify-between p-6 border-b border-zinc-100">
                                <h2 className="text-2xl font-black text-zinc-900 flex items-center gap-3">
                                    ...
                                    {currentLang === 'ar' ? 'سجل التحديثات' : currentLang === 'en' ? 'Changelog' : 'Journal des Mises à Jour'}
                                </h2>
                                <button onClick={() => setIsModalOpen(false)} className="p-2 bg-zinc-100/50 hover:bg-zinc-100 rounded-full text-zinc-500 transition-colors">
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                            <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-zinc-50/50">
                                {updates.length === 0 ? (
                                    <p className="text-center text-zinc-500 font-medium py-10">{t("Aucune mise à jour pour le moment.")}</p>
                                ) : (
                                    updates.map((up) => (
                            ...
                            <div className="p-6 bg-white border-t border-zinc-100 text-center">
                                <button onClick={() => setIsModalOpen(false)} className="px-8 py-3 bg-zinc-950 text-white rounded-full font-black text-sm uppercase tracking-widest rtl:tracking-normal hover:bg-zinc-800 transition-colors">
