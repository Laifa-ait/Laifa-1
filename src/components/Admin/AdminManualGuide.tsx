...
export const exportManualToPDF = (t: any) => {
  ...
  doc.setFontSize(9);
  doc.setTextColor(180, 83, 9);
  const securityWarning = doc.splitTextToSize(t("Ne partagez jamais vos identifiants d'administration supreme. Toute action est enregistree de maniere immuable dans les Audit Logs systeme et imputee a votre adresse email de session."), contentWidth - 10);
      ...
      <div className="bg-white rounded-[2.5rem] border border-zinc-200 p-8 shadow-sm relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-start gap-4">
             ...
             <div>
                <h3 className="text-xl font-black text-zinc-950 tracking-tight rtl:tracking-normal">{t("📖 Manuel de Gestion & Guide d'Utilisation OLMART")}</h3>
                <p className="text-zinc-500 text-xs font-semibold">{t("Le document technique complet pour piloter et comprendre l'intégralité du site OLMART.")}</p>
             </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
             ...
             >
                {isGuideOpen ? (
                <>{t("Masquer le manuel")}<ChevronUp className="w-4 h-4" /></>
                ) : (
                <>{t("Lire en ligne")}<ChevronDown className="w-4 h-4" /></>
                )}
             </button>
             <button 
             ...
             >
                <Download className="w-4 h-4" /> {t("Exporter PDF officiel")}</button>
          </div>
        </div>
        ...
        {isGuideOpen && (
          <div className="mt-8 border-t border-zinc-100 pt-8 space-y-6">
             <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex gap-3 text-amber-800 text-xs font-bold items-start">
                ...
                <div>
                <p className="uppercase tracking-wider rtl:tracking-normal font-black text-[9px] mb-1 text-amber-900">{t("Règle de Sécurité Interne (DevSecOps)")}</p>
                {t("Ce manuel est confidentiel et réservé à l'équipe de modération agréée OLMART. Ne l'imprimez ou ne le diffusez pas en dehors de vos canaux sécurisés.")}</div>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {sections.map((sec, idx) => (
                <div key={idx} className="bg-zinc-50 border border-zinc-100 p-6 rounded-[2rem] flex flex-col justify-between">
                   <div>
                      <h4 className="text-xs font-black text-[#ea580c] mb-3 uppercase tracking-wider rtl:tracking-normal">{sec.title}</h4>
