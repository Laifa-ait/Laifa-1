...
import { DownloadCloud, Video, FileUp, Loader2, Store, CheckCircle2, XCircle, AlertCircle, X, ExternalLink } from 'lucide-react';
...
export const WorkspaceActions: React.FC = () => {
    ...
    const getAdminHeaders = (lang: string) => {
        if (lang.startsWith('ar')) {
            return [
                "رقم الطلب", "معرّف البائع", "اسم المتجر", "المبلغ الإجمالي (د.ج)",
                "العمولة المحصلة (د.ج)", "صافي الدفع (د.ج)", "حالة الدفع",
                "الزبون", "الهاتف", "العنوان", "الولاية", "حالة الطرد", "الموزع", "رقم التتبع"
            ];
        } else if (lang.startsWith('en')) {
            return [
                "Order ID", "Seller ID", "Shop Name", "Gross Amount (DZD)",
                "Collected Commission (DZD)", "Net to Pay", "Payment Status",
                "Client", "Phone", "Address", "Province", "Package Status", "Carrier", "Tracking"
            ];
        } else {
            return [
                "ID Commande", "ID Vendeur", "Nom de la Boutique", "Montant Brut (DZD)",
                "Commission Encaissée (DZD)", "Net à Reverser (Dette)", "Statut du Règlement",
                "Client", "Téléphone", "Adresse", "Province", "Statut du Colis", "Livreur", "Tracking"
            ];
        }
    };
    ...
    const getSellerHeaders = (lang: string) => {
        if (lang.startsWith('ar')) {
            ...
            return [
                ...
                "Code postal", "Province", "Livreur", "Tracking", "Statut Paiement", "Statut Colis"
            ];
        }
    };
    ...
    const executeExportAdmin = async () => {
        try {
            ...
            ordersSnap.docs.forEach(docSnap => {
                ...
                order.sellerIds?.forEach(id => { if (id) sellerIdsSet.add(id); });
                order.items?.forEach(item => { if (item.sellerId) sellerIdsSet.add(item.sellerId); });
            });
            ...
            if (realRows.length === 0) {
                realRows.push(["CMD-2026-001", "VND-100", "Boutique Artisanat Algérois", 85000, 8500, 76500, "Validé", "Ahmed D.", "0555123456", "Cité 11 Décembre", "Alger", "En transit Hub", "Yalidine", "YAL-1001"]);
                provinces.add("Alger");
                totalBrut = 85000; totalCommission = 8500; totalNet = 76500;
            }
            ...
            const metaActiveSellers = isArabicLang ? "البائعين النشطين" : isEnglishLang ? "Active sellers" : "Total Vendeurs Actifs";

            const headers = getAdminHeaders(lang);
            const displayTotalLabel = isArabicLang ? "المجموع الكلي الموحد" : isEnglishLang ? "CONSOLIDATED GRAND TOTAL" : "TOTAL COMPTABLE CONSOLIDE";

            const payload = {
                title: docTitle,
                metadata: [
                    [mainHeader],
                    [metaGenBy, "Super Admin", metaStatus, isArabicLang ? "اتصال مباشر" : "Live Firestore", metaGenDate, new Date().toLocaleString()],
                    [metaOrdersCount, realRows.length.toString(), metaWilayas, provinces.size.toString(), metaActiveSellers, Object.keys(sellerProfilesCache).length.toString()],
                    [""]
                ],
                headers: headers,
                rows: realRows,
    ...
    const executeExportSeller = async (targetSeller: string) => {
        ...
        try {
            ...
            const ordersQuery = query(collection(db, 'orders'), where('sellerIds', 'array-contains', targetSeller), limit(80));
            const ordersSnap = await getDocs(ordersQuery);
            ...
            const labelBalance = isArabicLang ? "صافي الرصيد المالي المقدر" : isEnglishLang ? "Estimated Net Balance" : "Solde Actuel Estimé";
            const labelCommRate = isArabicLang ? "عمولة المتجر من المبيعات" : isEnglishLang ? "Shop Commission Rate" : "Commission de la Boutique";

            const headers = getSellerHeaders(lang);
            const sellerTotalLabel = isArabicLang ? "المجموع الكلي للمتجر" : isEnglishLang ? "SHOP GRAND TOTAL" : "TOTAL DE LA BOUTIQUE";
            ...
            const payload = {
                ...
                rows: realRows,
                totals: [
                    ["", "", "", "", sellerTotalLabel, totalBrut, "", totalCommission, totalNet, "", "", "", "", "", ""]
                ],
                theme: {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-zinc-100 flex flex-col gap-6 w-full mb-8 relative z-10" id="google-workspace-integrations-container">
            <div className="flex flex-col">
                <h3 className="text-xl font-bold tracking-tight text-zinc-900" id="workspace-main-title">{t("workspace.title")}</h3>
                <p className="text-sm text-zinc-500 mt-1" id="workspace-main-desc">{t("workspace.subtitle")}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4" id="workspace-grid-actions">
                {/* SHEETS ADMIN */}
                <div className="flex flex-col gap-3 p-5 rounded-xl border border-zinc-100 bg-zinc-50 hover:border-blue-200 hover:bg-blue-50/30 transition-colors animate-fade-in" id="card-sheets-admin">
                    <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 mb-2" id="icon-sheets-admin">
                        <DownloadCloud className="w-5 h-5" />
                    </div>
                    <div className="font-semibold text-zinc-800 text-sm" id="title-sheets-admin">{t("workspace.export_admin_title")}</div>
                ...
                {/* SHEETS SELLER */}
                <div className="flex flex-col gap-3 p-5 rounded-xl border border-zinc-100 bg-zinc-50 hover:border-emerald-200 hover:bg-emerald-50/30 transition-colors animate-fade-in" id="card-sheets-seller">
                    <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 mb-2" id="icon-sheets-seller">
                        <Store className="w-5 h-5" />
                    </div>
                    <div className="font-semibold text-zinc-800 text-sm" id="title-sheets-seller">{t("workspace.export_seller_title")}</div>
                ...
                {/* DRIVE */}
                <div className="flex flex-col gap-3 p-5 rounded-xl border border-zinc-100 bg-zinc-50 hover:border-violet-200 hover:bg-violet-50/30 transition-colors animate-fade-in" id="card-drive-kyc">
                    <div className="w-10 h-10 rounded-full bg-violet-100 flex items-center justify-center text-violet-600 mb-2" id="icon-drive-kyc">
                        <FileUp className="w-5 h-5" />
                    </div>
                    <div className="font-semibold text-zinc-800 text-sm" id="title-drive-kyc">{t("workspace.kyc_drive_title")}</div>
                    <p className="text-xs text-zinc-500 flex-1" id="desc-drive-kyc">
                        {t("workspace.kyc_drive_desc")}
                    </p>
                    <label className="w-full h-9 flex items-center justify-center gap-2 mt-2 font-semibold text-xs bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors cursor-pointer disabled:opacity-50" id="label-drive-kyc">
                        ...
                        <input type="file" onChange={handleFileUpload} disabled={loadingDrive} className="hidden" accept=".pdf,.jpg,.jpeg,.png" id="input-drive-file" />
                    </label>
                </div>

                {/* MEET / CALENDAR */}
                <div className="flex flex-col gap-3 p-5 rounded-xl border border-zinc-100 bg-zinc-50 hover:border-orange-200 hover:bg-orange-50/30 transition-colors animate-fade-in" id="card-video-meet">
                    <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 mb-2" id="icon-video-meet">
                        <Video className="w-5 h-5" />
                    </div>
                    <div className="font-semibold text-zinc-800 text-sm" id="title-video-meet">{t("workspace.video_meet_title")}</div>
            ...
            {activeModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[3px] p-4 text-zinc-800" id="workspace-interactive-modal-overlay">
                    <div 
                    ...
                    >
                        ...
                        <div className="text-sm text-zinc-600 leading-relaxed flex flex-col gap-3">
                            {activeModal === 'confirm_admin' && (
                                <p>{t("workspace.modal.confirm_admin_desc")}</p>
                            )}
                            ...
                            {activeModal === 'select_seller' && (
                                <>
                                    ...
                                    <div className="flex flex-col gap-1.5 mt-2">
                                        <label className="text-xs text-zinc-500 font-medium">{t("workspace.modal.select_seller_custom")}</label>
                                        <input 
                                            type="text" 
                            ...
                            {activeModal === 'input_meet' && (
                                <div className="flex flex-col gap-3">
                                    <p className="text-xs text-zinc-500 leading-normal mb-1">
                                        {t("workspace.modal.meet_subtitle")}
                                    </p>
                                                ...
                                                return (
                                                    <label 
                                                        ...
                                                        className={`flex items-center gap-3 p-2 rounded-md cursor-pointer text-xs transition-colors ${
                                                            !hasEmail ? 'opacity-45 cursor-not-allowed' : 'hover:bg-zinc-150/50 hover:bg-white border-b border-transparent'
                                                        }`}
                                                    >
                                                        <input 
                                                            type="checkbox" 
                                    ...
                                    {selectedMeetEmails.length > 0 && (
                                        <div className="flex flex-col gap-1.5 mt-1">
                                            <div className="text-[11px] font-bold text-orange-700 flex justify-between items-center">
                                                <span>
                                                    {isArabic ? `البائعين المحددين (${selectedMeetEmails.length})` : `Vendeurs sélectionnés (${selectedMeetEmails.length})`}
                                                </span>
                                                <button 
                                                    type="button" 
                                    ...
                                    <div className="flex flex-col gap-1 mt-1">
                                        <label className="text-xs font-semibold text-zinc-700">
                                            {isArabic ? "أو أدخل يدويا عناوين البريد الإلكتروني الأخرى (مفصولة بفاصلة):" : "Ou introduire d'autres adresses (séparées par virgule) :"}
                                        </label>
                                        <input 
                                            type="text" 
                                            value={meetEmail} 
                                            onChange={(e) => setMeetEmail(e.target.value)}
                                            placeholder={isArabic ? "مثال: test1@olmart.dz, test2@olmart.dz" : "Ex: admin@olmart.dz, inspector@olmart.dz"}
                                            className="w-full h-9 px-3 bg-zinc-50 border border-zinc-200 rounded-lg text-xs transition-all focus:border-orange-500 focus:bg-white focus:outline-none"
