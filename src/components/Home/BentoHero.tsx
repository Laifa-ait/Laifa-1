...
export const BentoHero: React.FC<{ banners: any[] }> = ({ banners }) => {
  ...
  if (!heroBanner) {
    return <div className="w-full min-h-[400px] bg-stone-200 animate-pulse rounded-3xl border border-[#EBE5DF]/40" />;
  }
  ...
  return (
    <div className="w-full min-h-[400px] relative rounded-3xl overflow-hidden group shadow-lg border border-stone-200">
      <img
        loading="lazy"
        src={
          getOptimizedImageUrl(heroBanner.desktop_image || heroBanner.imageUrl, 1200) ||
      ...
      <div className="absolute inset-0 p-6 md:p-12 flex flex-col justify-end items-start z-10">
        ...
        >
          ...
          <span className="font-mono text-xs rtl:text-sm uppercase tracking-widest rtl:tracking-normal text-[#121315]">
            {t("home.hero.exclusive_selection") || "DROP EXCLUSIF"}
          </span>
        </motion.div>
