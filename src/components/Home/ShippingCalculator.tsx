...
export const ShippingCalculator: React.FC = () => {
  ...
  const filteredWilayas = useMemo(() => {
    return ALGERIA_WILAYAS.filter((w) => w.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [searchQuery]);
  ...
  const tariffData = useMemo(() => {
    ...
    if (dbFee !== undefined) {
      ...
      } else if ([9, 35, 42, 15, 10].includes(code)) {
        // Blida, Boumerdes, Tipaza, Tizi, Bouira
        priceDomicile = 550;
        priceStopDesk = 350;
        delay = "48 heures";
      } else if ([31, 23, 25, 19, 13, 34, 18, 21].includes(code)) {
        // Oran, Annaba, Constantine, Setif, Tlemcen, BBA, Jijel, Skikda
        priceDomicile = 700;
        priceStopDesk = 400;
        delay = "2 à 3 jours";
      } else if ([1, 11, 33, 37, 47, 52, 53, 54, 55, 56, 57, 58].includes(code)) {
        // South Wilayas (Sahara)
        priceDomicile = 950;
        priceStopDesk = 600;
        delay = "4 à 6 jours";
      } else {
          ...
          <div className="mt-8 space-y-4 border-t border-white/10 pt-6 relative z-10">
            {[
              ...
              <div key={idx} className="flex gap-3">
                ...
                <div>
                  <h4 className="text-xs rtl:text-sm font-bold uppercase tracking-wider rtl:tracking-normal text-white">
                    {item.title}
                  </h4>
