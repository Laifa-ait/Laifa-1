...
export const FlashSales: React.FC = () => {
  ...
  if (loading) {
    return (
      <section className="py-8 sm:py-12 bg-white border-y border-[#C75C1A]/60">
        <div className="w-full max-w-[90rem] mx-auto px-4">
          <div className="w-full h-[400px] bg-stone-200 animate-pulse rounded-[2rem] border border-[#C75C1A]/40" />
        </div>
      </section>
    );
  }
    ...
    <section className="py-8 sm:py-12 relative -mt-4 bg-transparent border-none">
      <div className="w-full mx-auto px-1 sm:px-2 md:px-0">
        <div className="relative border-4 border-white bg-white p-5 sm:p-8 rounded-[3rem] sm:rounded-[4rem] shadow-xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5 mb-8 md:mb-10 pb-6 border-b-[3px] border-[#C75C1A]/20">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  {flashProducts.some((p) => p.flashSaleActive || p.promoPrice) && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#C75C1A] text-white text-[10px] sm:text-[11px] font-sans font-sans font-bold uppercase tracking-wider animate-bounce shadow-md">
                      <Flame className="w-3.5 h-3.5 fill-current animate-pulse" />
                      {t("home.flash.badge", "OFFRE LIMITÉE")}
                    </span>
                  )}
                  {flashProducts.some((p) => p.stock && p.stock > 0 && p.stock <= 20) && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-orange-100 text-[#C75C1A] text-[10px] sm:text-[11px] font-sans font-sans font-bold uppercase tracking-wider shadow-sm border border-orange-200">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-600 animate-ping" />
                      {t("stock_scarcity", "STOCK FAIBLE")}
                    </span>
                  )}
                </div>
                <h2 className="font-sans font-bold text-3xl sm:text-4xl text-[#2B1D15] uppercase leading-none drop-shadow-[0_2px_2px_rgba(0,0,0,0.05)]">
                  {t("home.flash.title", "VENTES FLASH")}
                </h2>
              </div>

              {/* Chrono Pill with exact h-11 height matching the button */}
              <div className="flex items-center justify-between sm:justify-start gap-4 bg-[#2B1D15] text-white px-4 rounded-full shadow-md h-12 shrink-0 w-full sm:w-auto mt-2 md:mt-0 border-2 border-white/20">
                <div className="flex items-center gap-2 shrink-0">
                  <Clock className="w-4 h-4 text-[#C75C1A] animate-spin" style={{ animationDuration: "4s" }} />
                  <span className="text-[10px] sm:text-[11px] font-sans font-bold text-[#FDF9EC] uppercase tracking-widest rtl:tracking-normal">
                ...
                <div className="flex items-center gap-1 font-sans font-bold text-sm sm:text-base">
                  ...
                  <span className="text-[#C75C1A] animate-pulse text-xs font-bold">:</span>
                  <span className="bg-white/20 px-2 py-0.5 rounded-lg text-white min-w-[2rem] text-center shadow-inner">
                    {String(timeLeft.minutes).padStart(2, "0")}
                  </span>
                  <span className="text-[#C75C1A] animate-pulse text-xs font-bold">:</span>
                  <span className="bg-[#C75C1A] px-2 py-0.5 rounded-lg text-white min-w-[2rem] text-center animate-pulse shadow-md border border-[#C75C1A]/40">
            ...
            <div className="w-full md:w-auto flex justify-end mt-2 md:mt-0">
              <button
                ...
                className="group flex gap-2 px-8 bg-[#C75C1A] hover:bg-[#C75C1A]/90 text-white font-sans font-bold text-sm uppercase items-center justify-center rounded-full shadow-md transition-all duration-300 hover:scale-105 active:scale-95 border-2 border-white h-12 shrink-0 w-full md:w-auto tracking-wide"
              >
          ...
          <div className="relative group/carousel -mx-2 sm:-mx-4 md:mx-0">
            ...
            >
              <ChevronLeft className="w-6 h-6 text-[#2B1D15]" />
            </button>
            <button
            ...
            >
              <ChevronRight className="w-6 h-6 text-[#2B1D15]" />
            </button>
            ...
            >
              {flashProducts.map((product, i) => {
                ...
                return (
                  ...
                  >
                    {/* The Product Card */}
                    <div className="bg-white rounded-2xl p-1.5 shadow-sm border border-orange-100 hover:border-[#C75C1A]/30 transition-all duration-300">
                      ...
                      {showProgress && (
                        <div className="mt-3 px-2 pb-2">
                          <div className="flex justify-between text-[10px] sm:text-[11px] font-sans font-bold text-[#2B1D15] mb-1">
                          ...
                          <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden border border-stone-200/50">
                            <div
                              className="h-full bg-gradient-to-r from-[#D98A50] to-[#C75C1A] rounded-full transition-all duration-1000"
