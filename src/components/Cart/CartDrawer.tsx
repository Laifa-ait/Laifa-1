      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-8 bg-[#FAF8F5]">
        {cart.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-6 px-4">
            ...
            >
              {t("Découvrir les tendances")}</button>
          </div>
        ) : (
          Object.values(groupedCart).map((group: any, groupIdx) => {
             ...
             return (
              <div key={groupIdx} className="bg-white rounded-3xl p-4 sm:p-5 shadow-sm border border-stone-100 flex flex-col gap-4">
                {/* Seller Header */}
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                         <span className="bg-[#121315] text-white text-[9px] rtl:text-[11px] font-black uppercase tracking-widest rtl:tracking-normal px-2 py-0.5 rounded-full">
                            {t("Sous-colis")}{groupIdx + 1}
                         </span>
                         <h3 className="font-black text-sm text-[#121315] uppercase tracking-widest rtl:tracking-normal leading-none">{group.sellerName}</h3>
                      </div>
                      <span className="text-xs rtl:text-sm font-bold text-stone-500">{group.items.length} {t("Article(s)")}</span>
                    </div>
                   ...
                   {remaining > 0 ? (
                     <div className="space-y-1.5">
                       <p className="text-[10px] rtl:text-[12px] sm:text-xs rtl:text-sm font-bold text-stone-500">
                         {t("Ajoutez encore")}<span className="text-[#F37021]">{formatPrice(remaining)}</span> {t("pour amortir la livraison !")}</p>
                       <div className="h-1.5 w-full bg-stone-100 rounded-full overflow-hidden">
                          <div className="h-full bg-[#F37021] transition-all duration-500" style={{ width: `${progress}%` }} />
                       </div>
                     </div>
                   ) : (
                     <div className="space-y-1.5">
                       <p className="text-[10px] rtl:text-[12px] sm:text-xs rtl:text-sm font-bold text-emerald-600">
                         {t("🎉 Excellent ! Vous optimisez vos frais de livraison chez ce vendeur.")}</p>
                       <div className="h-1.5 w-full bg-stone-100 rounded-full overflow-hidden">
                  ...
                  {group.items.map((item, i) => (
                    <div key={i} className="flex gap-4 group/item">
                      <div className="w-20 h-24 sm:w-24 sm:h-28 rounded-2xl overflow-hidden bg-stone-50 shrink-0 border border-stone-100 relative">
                        <img loading="lazy" src={getOptimizedImageUrl(item.image, 200)} className="w-full h-full object-cover" alt={getTranslatedField(item, 'name', lang)} />
                      </div>
                      <div className="flex-1 flex flex-col justify-between py-1">
                        <div>
                          <div className="flex justify-between items-start gap-2">
                            <h4 className="font-bold text-xs rtl:text-sm sm:text-sm text-[#121315] line-clamp-2">{getTranslatedField(item, 'name', lang)}</h4>
                            <button onClick={() => removeFromCart(item.originalIndex)} className="text-stone-300 hover:text-red-500 transition-colors p-1 bg-stone-50 rounded-full">
                        ...
                        <div className="flex items-center justify-between mt-2">
                           <div className="flex items-center gap-3 bg-stone-50 rounded-full px-2 py-1 border border-stone-200">
                              ...
                              <span className="text-xs rtl:text-sm font-black min-w-[12px] text-center text-[#121315]">{item.quantity}</span>
                              <button 
                                onClick={() => updateQuantity(item.originalIndex, item.quantity + 1)}
      ...
      {cart.length > 0 && (
        <div className="p-4 sm:p-6 border-t border-stone-100 space-y-4 bg-white">
          <div className="flex items-center justify-between text-sm sm:text-base font-medium">
            <span className="text-stone-400 font-bold uppercase tracking-widest rtl:tracking-normal text-[10px] rtl:text-[12px] sm:text-xs rtl:text-sm">{t("Sous-total")}</span>
          ...
          >
            {t("Passer à la caisse")}<ArrowRight className="w-4 h-4" />
          </button>
        </div>
      )}
    </SideDrawer>
