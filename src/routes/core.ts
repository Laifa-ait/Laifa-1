    ...
    try {
      ...
      for (const id of orderIds) {
        ...
        if (tStatus === "delivered" && cStatus !== "delivered") {
          const { deliveryPin, deliveryPhoto, latitude, longitude } = req.body;
          const orderPin = data.deliveryPin || "123456"; // Fallback for legacy orders
          ...
          if (!isUserAdmin && !hasValidPin && !hasValidPhoto) {
             return res.status(400).json({ 
                error: "La confirmation de livraison requiert soit le code PIN de l'acheteur (deliveryPin), soit une photo de livraison géolocalisée (deliveryPhoto, latitude, longitude)." 
             });
          }
    ...
    try {
      ...
      await db.runTransaction(async (transaction: any) => {
        ...
        if (availableBalance < amount) {
          throw new Error(`Solde disponible insuffisant (Solde disponible: ${availableBalance} DA, Solde verrouillé: ${lockedBalance} DA).`);
        }
