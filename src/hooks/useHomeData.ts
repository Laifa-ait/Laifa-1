  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      const fetchCategories = async () => {
        ...
        const snap = await withExponentialBackoff(() => getDocs(query(collection(db, "homepage_categories_v2"), limit(100))));
      ...
      const fetchCarousel = async () => {
        try {
           const cacheDocSnap = await withExponentialBackoff(() => getDocs(query(collection(db, "public"), where("__name__", "==", "homepage_cache"), limit(1))));
        ...
        const snapSections = await withExponentialBackoff(() => getDocs(query(collection(db, "homepage_sections"), orderBy("orderIndex", "asc"), limit(50))));
      ...
      const fetchProducts = async () => {
        ...
        const q = query(collection(db, "products"), where("status", "==", "active"), orderBy("createdAt", "desc"), limit(24));
        const snap = await withExponentialBackoff(() => getDocs(q));
        let productsLoaded = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }) as unknown as Product).filter(d => d && (d.stock === undefined || d.stock > 0));
