...
export const DeliveryTracking: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5]/20 pb-20">
      ...
      <div className="bg-[#121315] text-white py-14 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto space-y-4 text-center">
          <button 
            onClick={() => navigate("/")}
            className="inline-flex items-center gap-2 text-white/80 hover:text-white transition-colors text-xs font-black uppercase tracking-widest rtl:tracking-normal bg-white/10 px-4 py-2 rounded-xl mb-4 cursor-pointer border-none"
          >
            <ArrowLeft className="w-4 h-4" />
            {t("Retour à l'Accueil")}</button>
          <div className="flex items-center justify-center gap-2">
            <span className="bg-[#F37021] text-white text-[10px] font-black tracking-widest rtl:tracking-normal uppercase px-3 py-1 rounded-full shadow-md">
      ...
      <div className="max-w-4xl mx-auto px-4 mt-10 space-y-8">
        ...
        <div className="bg-white rounded-[2rem] border border-zinc-100 shadow-xl p-6 sm:p-10 text-center space-y-6">
          <h2 className="text-lg font-black text-[#121315] uppercase tracking-tight rtl:tracking-normal">
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-14 pr-6 py-4.5 bg-zinc-50 border border-zinc-200 focus:border-[#F37021] font-black text-[#121315] rounded-2xl outline-none shadow-inner"
              />
            </div>
            <button
              type="submit"
              className="px-8 py-4.5 bg-[#F37021] hover:bg-[#b04f30] text-white font-black text-xs uppercase tracking-widest rtl:tracking-normal rounded-2xl flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg cursor-pointer border-none"
            >
          ...
          <div className="pt-4 border-t border-zinc-50">
            <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal mb-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider rtl:tracking-normal transition-all border cursor-pointer ${selectedCarrier === carrier.id ? "bg-[#121315] text-white border-transparent shadow-md" : "bg-white text-zinc-500 border-zinc-200 hover:bg-zinc-50"}`}
                >
                  {carrier.label}
                </button>
              ))}
            ...
            >
              ...
              <div className="flex bg-zinc-100 rounded-2xl p-1 gap-1 max-w-sm mx-auto">
                <button
                  ...
                  className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rtl:tracking-normal rounded-xl transition-all cursor-pointer border-none ${activeTab === "visual" ? "bg-white text-[#121315] shadow-sm" : "text-zinc-500 hover:text-zinc-800"}`}
                >
                  {t("👁️ Suivi Client")}</button>
                <button
                  onClick={() => setActiveTab("technical")}
                  className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rtl:tracking-normal rounded-xl transition-all cursor-pointer border-none ${activeTab === "technical" ? "bg-white text-[#121315] shadow-sm" : "text-zinc-500 hover:text-zinc-800"}`}
                >
              ...
              {activeTab === "visual" ? (
                <>
                  ...
                  <div className="bg-[#121315] text-white rounded-[2rem] p-6 sm:p-8 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-48 h-48 bg-[#F37021]/10 rounded-full blur-2xl pointer-events-none" />
                    
                    <div className="space-y-3 relative z-10">
                      <div className="flex items-center gap-2">
                        <Barcode className="w-5 h-5 text-amber-400" />
                        <span className="text-xs font-black uppercase tracking-widest rtl:tracking-normal text-[#FAF8F5]/80">{t("Code d'expédition")}</span>
                      </div>
                      <h3 className="text-2xl font-black uppercase tracking-tight rtl:tracking-normal">{trackingData.id}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" />
                        <span className="text-xs font-black text-emerald-400 uppercase tracking-widest rtl:tracking-normal">{trackingData.currentStatusLabel}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/10 min-w-[200px] text-zinc-100 relative z-10 space-y-2">
                      <span className="block text-[8px] font-black uppercase tracking-widest rtl:tracking-normal text-zinc-300">{t("Détails Expédition")}</span>
                  ...
                  <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-xl p-6 sm:p-10 space-y-8">
                    <h3 className="text-base font-black text-[#121315] uppercase tracking-tight rtl:tracking-normal flex items-center gap-2">
                      <Layers className="w-5 h-5 text-[#F37021]" />
                    ...
                    <div className="relative pl-6 sm:pl-8 space-y-8 before:absolute before:left-[11px] sm:before:left-[15px] before:top-2 before:bottom-2 before:w-[2px] before:bg-zinc-100">
                      {trackingData.steps.map((step, i) => (
                        <div key={i} className="relative flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                          {/* Dot item */}
                          <div className={`absolute -left-[23px] sm:-left-[31px] top-1 w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center border-4 border-white shadow-md transition-all ${step.status === "completed" ? "bg-[#121315] text-white" : (step.status === "active" ? "bg-[#F37021] text-white animate-pulse" : "bg-zinc-100 text-zinc-350")}`}>
                          ...
                          <div className="space-y-1">
                            <h4 className={`text-sm font-black uppercase tracking-tight rtl:tracking-normal ${step.status === "active" ? "text-[#F37021]" : (step.status === "completed" ? "text-[#121315]" : "text-zinc-400")}`}>
                              {step.title}
                            </h4>
                            <p className="text-xs text-zinc-500 font-semibold leading-relaxed">
                              {step.desc}
                            </p>
                            <span className="inline-block text-[9px] font-black text-[#121315]/40 uppercase tracking-widest rtl:tracking-normal bg-zinc-50 px-2 py-0.5 rounded-md mt-1">
                              📍 {step.hub}
                            </span>
                          </div>
                  ...
                  {/* Courier detail info */}
                  <div className="bg-orange-650/[0.03] border border-[#F37021]/10 rounded-[2rem] p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="block text-[8px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal">{t("Livreur Assigné")}</span>
                        <h4 className="text-sm font-black text-[#121315] uppercase tracking-tight rtl:tracking-normal">{trackingData.courierName}</h4>
                        <p className="text-xs text-zinc-500 font-medium leading-none mt-1">{t("Livreur régional agréé •")}{trackingData.wilaya}</p>
                      </div>
                    </div>

                      <div className="px-5 py-3.5 bg-white border border-zinc-200 text-zinc-400 font-black text-xs uppercase tracking-widest rtl:tracking-normal rounded-xl flex items-center justify-center gap-2 shadow-sm">
                ...
                <div className="bg-zinc-950 text-zinc-100 rounded-[2.5rem] p-6 sm:p-10 space-y-6 shadow-xl font-mono">
                  <div className="flex items-center justify-between border-b border-white/10 pb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs uppercase font-black text-orange-400">{t("Yalidine & Mayestro Direct Hook")}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] uppercase font-black px-2 py-1 rounded bg-white/15 text-zinc-300">
                  ...
                  <div className="space-y-4 pt-2">
                    <span className="block text-[10px] font-black text-amber-500 uppercase">{t("1. Exemple de Hook Payload attendu (JSON) :")}</span>
...
{`{
  ...
  "recipient": {
    ...
    "cod_to_collect": ${trackingData.codAmount}
                    ...
                    <span className="block text-[10px] font-black text-amber-500 uppercase">{t("2. Endpoints requis pour le raccordement physique :")}</span>
                    <div className="space-y-2 text-xs font-bold font-sans">
                      <div className="flex items-center gap-2 bg-white/5 p-3 rounded-lg">
                        <span className="bg-blue-600 font-black text-[9px] px-2 py-0.5 rounded text-white font-mono">{t("POST")}</span>
                        <span className="text-zinc-300 font-mono text-[10px]">{t("/api/v1/shipment/create-ticket")}</span>
                      </div>
                      <div className="flex items-center gap-2 bg-white/5 p-3 rounded-lg">
                        <span className="bg-emerald-600 font-black text-[9px] px-2 py-0.5 rounded text-white font-mono">{t("GET")}</span>
            ...
            searchedId && (
              <div className="bg-white rounded-[2rem] p-10 text-center space-y-4 border border-zinc-100 shadow-md">
                ...
                <h3 className="text-lg font-black text-[#121315] uppercase tracking-tight rtl:tracking-normal">{t("Aucun envoi trouvé")}</h3>
        ...
        <div className="bg-zinc-100 rounded-[2rem] p-6 text-center space-y-4">
          <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal">
