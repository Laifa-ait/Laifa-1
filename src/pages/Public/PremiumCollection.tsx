...
export const PremiumCollection: React.FC = () => {
  ...
  // Section dynamic customization states
  ...
  // Robust string cleaner for accent-insensitive, case-insensitive comparison
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // 1. Fetch active homepage sections to find matching one (normalize IDs, names, and titles to prevent mismatch of capitalization or accents)
        ...
        // 2. Fetch banners as a fallback or parallel source to see if we match custom banner page
        ...
        // Resolve Section Meta information (Clean title and custom background)
        if (matchingSection) {
          ...
          if (matchingSection.themeImage) {
            ...
            try {
              ...
              console.error("Error loading seasonal theme in PremiumCollection:", themeErr);
            }
          }
        ...
        // If no curated products yet but it has a specific tag/category configured
        if (curatedProducts.length === 0) {
          const productsRef = collection(db, "products");
          if (sectionCategory) {
            const catQ = query(productsRef, where("category", "==", sectionCategory), limit(24));
            const catSnap = await getDocs(catQ);
            curatedProducts = catSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as Product));
          } else if (sectionTag) {
            const tagQ = query(productsRef, where("tags", "array-contains", sectionTag), limit(24));
            const tagSnap = await getDocs(tagQ);
            curatedProducts = tagSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as Product));
          }
        }

        // 2. Discover tags to fetch related products sharing the same tags
        ...
        // Fetch other approved products so we can sort them
        const productsRef = collection(db, "products");
        ...
        // Products that share ANY tag of the curated products/section
        ...
        // Other products as downstream remainder
        const remainderProducts = allProducts.filter(p => {
          return !curatedIds.has(p.id) && !sameTagProducts.some(st => st.id === p.id);
        });

        // Combine into the requested sequence: Curated first, same tags second, others third
        ...
        try {
          const productsRef = collection(db, "products");
          const q = query(productsRef, where("status", "==", "active"), limit(24));
          const snapshot = await getDocs(q);
          const data = snapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
          ...
          console.error("Dual fallback error:", e);
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-[#FDF9EC]" ref={containerRef}>
      <div className="h-[25vh] min-h-[220px] md:h-[35vh] md:min-h-[320px] relative flex flex-col justify-end">
        {/* Transparent Custom Header (high z-index for clickability) */}
        <div className="absolute top-0 left-0 w-full p-3 lg:p-4 flex justify-between items-center z-[65]">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-md transition-all cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <div className="font-bold text-lg sm:text-xl tracking-widest rtl:tracking-normal text-white drop-shadow-md uppercase">
            {t("Olmart")}</div>
        ...
        {/* Background Layer with overflow hidden to not clip search dropdown later */}
        <div className="absolute inset-0 z-0 overflow-hidden">
          <motion.img
            src={sectionBannerImg || "https://images.unsplash.com/photo-1600607686527-6fb886090705?q=80&w=2000&auto=format&fit=crop"}
          ...
          <motion.div style={{ y: textY, opacity: textOpacity }}>
            <div className="flex items-center gap-2 text-[9px] sm:text-xs font-bold tracking-widest rtl:tracking-normal text-white/80 uppercase mb-1 md:mb-2">
              <Link to="/" className="hover:text-white transition-colors">
                {t("Accueil")}</Link>
              <span>/</span>
              <span>{t("Collection Premium")}</span>
            </div>
      ...
      <div ref={filterSectionRef} className="relative z-40 bg-[#FDF9EC] rounded-t-[2rem] -mt-6 sm:-mt-8 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] min-h-[65vh]">
        {/* Barre d'outils et catégories (Filtres Express) */}
        <div className="sticky top-0 z-40 bg-[#FDF9EC]/95 backdrop-blur-md border-b border-[#3C2B22]/10 shadow-sm rounded-t-[2rem] pt-6 sm:pt-8">
          <div className="max-w-[90rem] mx-auto px-4 sm:px-8 pb-3 lg:pb-4">
            <div className="flex flex-col gap-3">
              {/* Top row: Categories & Fixed Filters Button */}
              ...
              <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide no-scrollbar pb-1">
                ...
                <div className="relative ms-auto shrink-0 z-[100]">
                  ...
                  {showSortDropdown && (
                    <>
                      {/* Invisible backdrop to dismiss sort panel */}
                      <div className="fixed inset-0 z-40" onClick={() => setShowSortDropdown(false)} />
                      
                      <div className="absolute right-0 top-full mt-1.5 bg-white border border-[#3C2B22]/10 rounded-xl shadow-xl z-50 py-1.5 min-w-[150px] animate-in fade-in-50 slide-in-from-top-1 rtl:left-0 rtl:right-auto">
                        {[
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-8 py-8 sm:py-12 pb-24">
          ...
          {loading ? (
            ...
            <div className="text-center text-[#3C2B22]/50 py-20 text-lg">
              {t("Aucun article trouvé pour cette collection.")}</div>
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {showAdvancedFilters && (
          <>
            ...
            {/* Slide-over Drawer (Right side for desktop, bottom/full for mobile) */}
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="fixed inset-y-0 right-0 w-full max-w-md bg-[#FAF6EE] shadow-2xl z-[201] flex flex-col overflow-hidden text-[#3C2B22]"
            >
              {/* Header */}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#3C2B22] pointer-events-none" />
                  </div>
                </div>
                ...
                {/* Express Filters Options / Toggle */}
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <div className={`text-[10px] font-black uppercase tracking-widest rtl:tracking-normal ${isActive ? "text-[#FF5C00]" : "text-[#3C2B22]"}`}>{opt.name}</div>
                            <div className="text-[9px] text-[#3C2B22]/60 mt-0.5 leading-relaxed font-semibold">{opt.desc}</div>
                          </div>
                        </button>
                      );
                    })}
