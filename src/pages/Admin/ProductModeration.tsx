...
import { collection, query, where, getDocs, updateDoc, doc, Timestamp, orderBy, deleteDoc, addDoc, serverTimestamp } from 'firebase/firestore';
import { CheckCircle2, XCircle, Search, Filter, Sparkles, Star, Tag, ShoppingBag, Eye, ShieldAlert, FileText, RefreshCw, X, Trash2, ArrowLeft } from 'lucide-react';
...
export const ProductModeration: React.FC = () => {
  ...
  const handleApprove = async (product: any) => { const toastId = toast.loading(t('Approbation en cours...')); try { const token = await currentUser?.getIdToken(true); if (!token) throw new Error('Non authentifié'); const res = await fetch('/api/admin/products/' + product.id + '/approve', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' } }); if (!res.ok) throw new Error('Erreur serveur'); const data = await res.json(); setProducts(prev => prev.filter(p => p.id !== product.id)); toast.success(isArabic ? 'تمت الموافقة على المنتج ' + product.name + ' بنجاح!' : 'Produit ' + product.name + ' approuvé avec succès !', { id: toastId }); } catch (err) { console.error(err); toast.error(t('Erreur lors de l\'approbation du produit.'), { id: toastId }); } };
  ...
  const handleRejectSubmit = async () => { if (!targetProduct) return; const finalReason = rejectReason === 'Autre reason' ? customReason : rejectReason; if (!finalReason.trim()) { toast.error(t('Veuillez renseigner ou sélectionner un motif de refus.')); return; } const toastId = toast.loading(t('Rejet en cours...')); try { const token = await currentUser?.getIdToken(true); if (!token) throw new Error('Non authentifié'); const res = await fetch('/api/admin/products/' + targetProduct.id + '/reject', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: finalReason }) }); if (!res.ok) throw new Error('Erreur serveur'); setProducts(prev => prev.filter(p => p.id !== targetProduct.id)); setRejectModalOpen(false); setTargetProduct(null); toast.success(isArabic ? 'تم رفض المنتج ' + targetProduct.name + '.' : 'Le produit ' + targetProduct.name + ' a été rejeté.', { id: toastId }); } catch (err) { console.error(err); toast.error(t('Erreur lors du rejet du produit.'), { id: toastId }); } };
  ...
  const handleConfirmDelete = async (product: any) => {
    ...
    if (window.confirm(confirmationText)) {
      try {
        ...
        toast.success(
          t("Le produit a été définitivement supprimé.")
        );
      } catch (err) {
        console.error("Error deleting product:", err);
        toast.error(
          t("Erreur lors de la suppression définitive.")
        );
      }
    }
  };
  ...
  const handleDenyDelete = async (product: any) => {
    ...
    if (window.confirm(confirmationText)) {
      try {
        ...
        setProducts(prev => prev.filter(p => p.id !== product.id));
        toast.success(
          t("La suppression a été refusée et le produit est réactivé.")
        );
      } catch (err) {
        console.error("Error setting product as active:", err);
        toast.error(
          t("Erreur lors de la réactivation.")
        );
      }
    }
  };
 
  const handleRecalculateScores = async () => { if (activeTab !== 'active') return; setLoading(true); const toastId = toast.loading(t('Recalcul de tous les scores de pertinence...')); try { const token = await currentUser?.getIdToken(true); if (!token) throw new Error('Non authentifié'); const res = await fetch('/api/admin/products/recalculate-scores', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token } }); if (!res.ok) throw new Error('Erreur serveur'); await fetchProducts(); toast.success(t('Tous les Scores de Pertinence ont été recalculés dynamiquement selon la formule de l\'audit !'), { id: toastId }); } catch (err) { console.error(err); toast.error(t('Erreur durant recalcul.'), { id: toastId }); } finally { setLoading(false); } };
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-stone-200">
        <div>
          <h1 className="text-3xl font-black tracking-tight rtl:tracking-normal text-zinc-900 uppercase">{t("Modération des Produits")}</h1>
          <p className="text-xs text-zinc-500 font-semibold tracking-wider rtl:tracking-normal mt-1 uppercase">{t("Gouvernance Premium du Catalogue Olma")}</p>
        </div>
        <div className="flex items-center gap-3">
          {activeTab === 'active' && (
            ...
            >
              ...
              {t("Recalculer les Scores de Pertinence")}</button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="relative col-span-2">
          ...
          <input
            ...
            placeholder={t("Rechercher par titre, marque ou vendeur...") || "Rechercher par titre, marque ou vendeur..."}
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="bg-white border border-stone-200/60 rounded-2xl p-16 text-center shadow-sm">
          ...
          <h2 className="text-sm font-black text-zinc-800 uppercase tracking-wider rtl:tracking-normal">{t("Aucun produit trouvé")}</h2>
          <p className="text-xs text-zinc-400 mt-1 max-w-md mx-auto">{t("Il n'y a aucun produit conforme aux critères de recherche dans cet onglet de modération.")}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((p) => {
              ...
              const score = p.qualityScore !== undefined ? p.qualityScore : parseFloat((((p.salesCount || 0) * 10) + ((p.viewsCount || 0) * 0.1) + ((p.sellerRating || 4.5) * 5) - ((p.rtoRate || 0) * 50)).toFixed(2));
                ...
                >
                  ...
                  <div className="aspect-[4/3] bg-zinc-50 relative overflow-hidden shrink-0 border-b border-stone-100">
                    ...
                    <div className="absolute top-4 start-4 flex flex-col gap-2">
                      {p.status === 'pending' && (
                        <>
                          <span className="px-3 py-1.5 w-fit rounded-lg bg-orange-100 text-orange-800 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal">{t("⏳ En attente de modération")}</span>
                          {p.moderationType === 'update' ? (
                            <span className="px-3 py-1.5 w-fit rounded-lg bg-blue-100 text-blue-800 border border-blue-500/20 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal shadow-sm flex items-center gap-1.5"><RefreshCw size={12} className="inline-block" /> {t("Produit Modifié")}</span>
                          ) : (
                            <span className="px-3 py-1.5 w-fit rounded-lg bg-purple-100 text-purple-800 border border-purple-500/20 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal shadow-sm flex items-center gap-1.5"><Sparkles size={12} className="inline-block" /> {t("Nouveau Produit")}</span>
                          )}
                        </>
                      )}
                      {p.status === 'active' && <span className="px-3 py-1.5 w-fit rounded-lg bg-emerald-100 text-emerald-800 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal">{t("✅ Actif")}</span>}
                      {p.status === 'rejected' && <span className="px-3 py-1.5 w-fit rounded-lg bg-red-100 text-red-800 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal">{t("❌ Refusé")}</span>}
                      {p.status === 'pending_deletion' && <span className="px-3 py-1.5 w-fit rounded-lg bg-red-100 text-red-800 text-[9px] font-black uppercase tracking-wider rtl:tracking-normal animate-pulse">{t("🗑️ Suppression Demandée")}</span>}
                    </div>
                  </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-stone-400 uppercase tracking-widest rtl:tracking-normal leading-none">{p.category || 'Sans catégorie'}</span>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-[9px] font-bold text-zinc-500">{t("Vendeur:")}<b className="text-zinc-700 underline">{p.sellerName || "Inconnu"}</b></span>
                      </div>
                      ...
                      {p.rejectionReason && activeTab === 'rejected' && (
                        <div className="mt-3 p-3 bg-red-50 rounded-xl border border-red-100 text-red-800 text-[10px] font-semibold">
                          {t("Motif de rejet:")}<span className="font-normal italic">"{p.rejectionReason}"</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-[#FAF8F6] p-3 rounded-xl border border-stone-200/50">
                      <div className="text-center">
                        <span className="text-[8px] font-black text-stone-400 uppercase tracking-wide">{t("Score de Qualité")}</span>
                      ...
                      <div className="text-center border-l border-stone-300/40">
                        <span className="text-[8px] font-black text-stone-400 uppercase tracking-wide">{t("Prix de Vente")}</span>
            ...
            >
              <div className="flex items-center justify-between pb-4 border-b border-stone-100">
                <h3 className="text-sm font-black text-zinc-900 uppercase tracking-wider rtl:tracking-normal flex items-center gap-2">
                  ...
                  {t("Sélectionner un motif de rejet")}</h3>
                <button
                  onClick={() => setRejectModalOpen(false)}
              ...
              <div className="space-y-4 mt-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest rtl:tracking-normal">{t("Raisons fréquentes :")}</label>
                  ...
                  >
                    {t("Autre raison personnalisée...")}</button>
                </div>

                {rejectReason === "Autre reason" && (
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest rtl:tracking-normal">{t("Saisir le motif :")}</label>
                    <textarea
                      rows={3}
                      ...
                      placeholder={t("Indiquez clairement le problème au vendeur...") || "Indiquez clairement le problème au vendeur..."}
                      value={customReason}
