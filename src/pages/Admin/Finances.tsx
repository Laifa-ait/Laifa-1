...
import { Wallet, History, Landmark, CheckCircle2, XCircle, Clock, AlertCircle, FileSpreadsheet, ArrowDownToLine, Upload, X, Loader2, Save, Percent } from 'lucide-react';
...
export const Finances: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 500 * 1024) { // 500kb limit for base64 mockup
  ...
  const handleMarkAsPaid = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setWithdrawals(withdrawals.map(w => w.id === selectedWithdrawal.id ? { ...w, status: 'PAID', receiptUrl: receiptValue } : w));
  ...
  const totalPendingAmount = withdrawals.filter(w => w.status?.toLowerCase() === 'pending').reduce((acc, curr) => acc + curr.amount, 0);

  return (
    <div className="space-y-12 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black tracking-tight rtl:tracking-normal text-zinc-950">{t("Portefeuille & Litiges")}</h2>
          <p className="text-zinc-500 font-medium">{t("Gérez les flux financiers et les paiements vendeurs.")}</p>
        </div>
        <button onClick={exportToCSV} className="px-8 py-5 bg-[#ea580c] text-white rounded-[2rem] flex items-center gap-4 font-black text-[11px] uppercase tracking-widest rtl:tracking-normal shadow-xl shadow-orange-500/20">
           <FileSpreadsheet className="w-5 h-5" />
           {t("Export Comptable (Excel)")}</button>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
         <div className="bg-white rounded-[3rem] p-10 relative overflow-hidden border border-zinc-100 shadow-xl md:col-span-2 flex flex-col justify-between">
            <div>
               <div className="flex items-start justify-between mb-2">
                  <div>
                     <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest rtl:tracking-normal mb-2">{t("Commission Globale")}</h4>
                     <h3 className="text-xl font-black text-zinc-950 tracking-tight rtl:tracking-normal">{t("Taux appliqué par défaut aux vendeurs")}</h3>
                  </div>
         ...
         <div className="bg-zinc-950 rounded-[3rem] p-10 text-white relative overflow-hidden border border-white/10 shadow-2xl">
            ...
            <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest rtl:tracking-normal mb-2 relative z-10">{t("En attente de virement")}</h4>
            <h3 className="text-4xl font-black text-[#ffffff] tracking-tighter rtl:tracking-normal mb-4 relative z-10">{formatPrice(totalPendingAmount)}</h3>
            <div className="flex items-center gap-2 relative z-10">
               <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
               <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal">{pendingCount} {t("demandes en cours")}</span>
            </div>
         </div>
      </div>
      ...
      <div className="bg-white rounded-[3.5rem] border border-zinc-100 shadow-sm overflow-hidden">
         <div className="p-10 border-b border-zinc-50">
            <h4 className="text-xl font-black flex items-center gap-4">
               ...
               {t("Demandes de Retrait")}</h4>
         </div>
         <div className="divide-y divide-zinc-50">
            {withdrawals.length === 0 ? (
               <div className="p-24 text-center">
                  <Wallet className="w-20 h-20 text-zinc-100 mx-auto mb-6" />
                  <p className="text-zinc-400 font-bold uppercase tracking-widest rtl:tracking-normal text-xs">{t("Aucune demande de retrait.")}</p>
               </div>
            ) : (
               withdrawals.map((w) => {
                 
                 return (
                                 <div key={w.id} className="p-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10 hover:bg-zinc-50/30 transition-colors group">
                                    <div className="flex items-center gap-8">
                                       <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center ${['PAID', 'COMPLETED'].includes(w.status?.toUpperCase() || '') ? 'bg-emerald-50 text-emerald-600' : w.status?.toUpperCase() === 'FAILED' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'}`}>
                                          <Landmark className="w-8 h-8" />
                                       </div>
                                       <div>
                                          <h4 className="text-2xl font-black text-zinc-950 tracking-tighter rtl:tracking-normal mb-1">{formatPrice(w.amount)}</h4>
                                          <div className="flex items-center gap-3">
                                             <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal">{w.shopName || (w as any).sellerName || 'Boutique'}</span>
                                             <span className="w-1 h-1 bg-zinc-200 rounded-full" />
                                             <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal">{t("RIB/CCP:")}{w.accountDetails || (w as any).rib}</span>
                                          </div>
                                       </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6">
                                       <div className="text-end hidden lg:block">
                                          <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest rtl:tracking-normal mb-1">{t("Date Demande")}</p>
                                          <p className="text-sm font-black text-zinc-950">{w.createdAt?.toDate().toLocaleDateString()}</p>
                                       </div>
                                       {w.status?.toLowerCase() === 'pending' ? (
                                          <button 
                                            onClick={() => { setSelectedWithdrawal(w); setReceiptValue(""); }}
                                          ...
                                          >
                                             <CheckCircle2 className="w-4 h-4" /> {t("Traiter Virement")}</button>
                                       ) : (
                                          <div className="flex items-center gap-3 px-8 py-4 bg-emerald-50 text-emerald-600 rounded-2xl font-black text-[10px] uppercase tracking-widest rtl:tracking-normal border border-emerald-100">
                                             <CheckCircle2 className="w-4 h-4" /> {t("Virement Effectué")}</div>
                                       )}
                                    </div>
                                 </div>
                              );
      ...
      <AnimatePresence>
         {selectedWithdrawal && (
             <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedWithdrawal(null)} className="absolute inset-0 bg-zinc-950/60" />
                 
                 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-8 overflow-hidden z-10">
                     <button onClick={() => setSelectedWithdrawal(null)} className="absolute top-6 end-6 text-zinc-400 hover:text-zinc-950 transition-colors">
                         <X className="w-6 h-6" />
                     </button>

                     <h3 className="text-2xl font-black text-zinc-900 tracking-tight rtl:tracking-normal mb-2">{t("Confirmer le paiement")}</h3>
                     ...
                     <div className="bg-zinc-50 rounded-2xl p-4 mb-6 border border-zinc-100">
                         <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest rtl:tracking-normal mb-1">{t("Montant à transférer")}</p>
                         <p className="text-2xl font-black text-zinc-900 mb-2">{formatPrice(selectedWithdrawal.amount)}</p>
                         <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest rtl:tracking-normal mb-1">{t("Coordonnées (")}{(selectedWithdrawal.method || "").replace('_', ' ')})</p>
                         <p className="text-sm font-bold text-zinc-900">{selectedWithdrawal.accountDetails || (selectedWithdrawal as any).rib}</p>
                     </div>
                     ...
                     <form onSubmit={handleMarkAsPaid} className="space-y-6">
                        <div>
                               <div>
                                 <div className="relative overflow-hidden w-full px-5 py-4 bg-zinc-50 border border-zinc-200 border-dashed rounded-2xl flex items-center justify-center hover:bg-zinc-100 transition-colors cursor-pointer">
                                    <input type="file" required accept="image/*,application/pdf" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                    <div className="flex flex-col items-center text-center gap-2 pointer-events-none">
                                        <Upload className="w-6 h-6 text-zinc-400" />
                                        <span className="text-xs font-bold text-zinc-500">
                                            {receiptValue ? "Fichier chargé (Cliquez pour changer)" : "Cliquez ou glissez une image preuve ici"}
                                        </span>
                                    </div>
                                 </div>
                               </div>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            >
                                {t("Échec du Virement / Rejeter")}</button>
                        </div>
                     </form>
                 </motion.div>
             </div>
