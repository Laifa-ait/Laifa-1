...
export const AgentsAdmin: React.FC = () => {
    ...
    const renderAgentStatus = (isActive: boolean) => {
      return (
            <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rtl:tracking-normal rounded-full border flex items-center gap-1 shadow-xs transition-colors ${
                isActive 
                    ? 'bg-green-50 text-green-700 border-green-200' 
                    : 'bg-zinc-100 text-zinc-500 border-zinc-200'
            }`}>
                {isActive ? <><CheckCircle2 className="w-3 h-3" /> {t("Actif")}</> : 'Désactivé'}
            </span>
        );
    };
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-32 pt-8">
            <div className="flex flex-col gap-2">
                ...
                <h1 className="text-3xl font-black text-zinc-950 uppercase tracking-tight rtl:tracking-normal">{t("Agents IA")}</h1>
                <p className="text-zinc-500 font-medium text-sm max-w-2xl">
                    {t("Gérez vos agents d'intelligence artificielle spécialisés pour automatiser des processus complexes et augmenter les performances de votre marketplace.")}</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h3 className="text-sm font-bold text-blue-600 mb-4 tracking-wider rtl:tracking-normal uppercase">{t("Analyste Stratégique")}</h3>
                        
                        <p className="text-zinc-500 text-sm leading-relaxed mb-8 min-h-[4rem]">
                            {t("Il analyse en continu les tendances du marché, les mots-clés les plus recherchés, et les comportements des clients pour optimiser les prix et cibler les campagnes avec précision.")}</p>
                        
                        <div className="pt-6 border-t border-zinc-100 flex items-center justify-between">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider rtl:tracking-normal mb-1">{t("Impact")}</span>
                                <span className="text-sm font-black text-zinc-800">{t("+14% Conversion")}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleConfigureClick('Growth Analyst')} className="w-10 h-10 flex items-center justify-center bg-zinc-100 text-zinc-600 rounded-xl hover:bg-zinc-200 transition-colors">
                                ...
                                >
                                    {loadingAgent === 'growth' ? <div className="w-4 h-4 border-2 border-current border-t-transparent animate-spin rounded-full" /> : (configs.growth ? 'Désactiver' : 'Activer')}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h3 className="text-sm font-bold text-orange-600 mb-4 tracking-wider rtl:tracking-normal uppercase">{t("Emailing / Notifications Mails")}</h3>
                        
                        <p className="text-zinc-500 text-sm leading-relaxed mb-8 min-h-[4rem]">
                            {t("Il traque les paniers abandonnés et envoie automatiquement des relances ciblées par email avec des offres personnalisées pour convertir les hésitations en achats.")}</p>
                        ...
                        <div className="pt-6 border-t border-zinc-100 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleConfigureClick('Récupération Paniers')} className="w-10 h-10 flex items-center justify-center bg-zinc-100 text-zinc-600 rounded-xl hover:bg-zinc-200 transition-colors">
                                    <Settings className="w-4 h-4" />
                                </button>
                                <button 
                                ...
                                >
                                    {loadingAgent === 'cart' ? <div className="w-4 h-4 border-2 border-current border-t-transparent animate-spin rounded-full" /> : (configs.cart ? 'Désactiver' : 'Activer')}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h3 className="text-sm font-bold text-emerald-600 mb-4 tracking-wider rtl:tracking-normal uppercase">{t("Vérification Automatique")}</h3>
                        
                        <p className="text-zinc-500 text-sm leading-relaxed mb-8 min-h-[4rem]">
                            {t("Examine les fiches produits créées par les vendeurs et les avis des clients pour détecter les anomalies, les fraudes ou les contenus inappropriés avant publication.")}</p>
                        
                        <div className="pt-6 border-t border-zinc-100 flex items-center justify-between">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider rtl:tracking-normal mb-1">{t("Objectif")}</span>
                                <span className="text-sm font-black text-zinc-800">{t("Conformité")}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleConfigureClick('Modérateur')} className="w-10 h-10 flex items-center justify-center bg-zinc-100 text-zinc-600 rounded-xl hover:bg-zinc-200 transition-colors">
                                    <Settings className="w-4 h-4" />
                                </button>
                                <button 
                                ...
                                >
                                    {loadingAgent === 'moderator' ? <div className="w-4 h-4 border-2 border-current border-t-transparent animate-spin rounded-full" /> : (configs.moderator ? 'Désactiver' : 'Activer')}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h3 className="text-sm font-bold text-purple-600 mb-4 tracking-wider rtl:tracking-normal uppercase">{t("Chatbot / Support Niveau 1")}</h3>
                        
                        <p className="text-zinc-500 text-sm leading-relaxed mb-8 min-h-[4rem]">
                            {t("Prend en charge les demandes fréquentes des clients (suivi de commande, politiques de retour) via le chatbot et ne redirige vers un humain qu'en cas de litige complexe.")}</p>
                        ...
                        <div className="pt-6 border-t border-zinc-100 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleConfigureClick('Support Patient')} className="w-10 h-10 flex items-center justify-center bg-zinc-100 text-zinc-600 rounded-xl hover:bg-zinc-200 transition-colors">
                                    <Settings className="w-4 h-4" />
                                </button>
                                <button 
                                ...
                                >
                                    {loadingAgent === 'support' ? <div className="w-4 h-4 border-2 border-current border-t-transparent animate-spin rounded-full" /> : (configs.support ? 'Désactiver' : 'Activer')}
                                </button>
                            </div>
                        </div>
                    </div>
